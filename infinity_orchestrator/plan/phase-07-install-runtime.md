...
- **What:** Inside the orchestrator's `Step5Action::StartInstall` dispatcher (called by `page_workspace_step5::render`'s action handler and by `stage_installing::render`'s analogous dispatcher), before flipping `state.step5.start_install_requested = true`:
  ...
  2b. **For share-code-consuming workflows (Install Modlist / Create-import / Load-Draft), invoke P7.T17 here:** derive the per-install directories inside the destination + force the clone flags (per SPEC §13.12a / §13.12 #3/#4), then kick off the import → auto-build → download/extract pipeline through the content-addressed staging layer. `on_install_start` is the trigger point; P7.T17 owns the mechanism. (Fresh Create → New-from-downloaded-mods skips the import step but still gets the per-install dirs.)
