...
| `src/ui/settings/tab_tools.rs` | Renders writable PathRows for `weidu` and `mod_installer`, bound to the matching `Step1Settings::*_binary` fields. | widgets, settings store |
...
| `src/settings/redesign_store.rs` | `pub struct RedesignSettingsStore { path }` with `load` / `save` / `backup_corrupt_file` mirroring `RegistryStore`. Persists `bio_redesign_settings.json` in the same config dir. `load` is pure (never mutates the file); a corrupt file is an `Err` and the caller backs it up + falls back to defaults per SPEC §13.14. | `redesign_fields`, platform_defaults |
...
- **What:** Writable PathRows for `WeiDU binary` and `Mod installer`, bound to `Step1Settings::{weidu,mod_installer}_binary`. The row uses the same two-line layout as Paths (label + input + browse + status line). `validate_now::check_binary` runs `resolve_on_path` for bare names so the row tells the truth: bare name not on `$PATH` → `Error { reason: "not on $PATH — install or specify the full path" }` (red border); bare name on `$PATH` → `Ok { detail: Some(resolved absolute path) }` so the user sees which binary will actually run; absolute path → `is_file` check.
- **Where:** New file `src/ui/settings/tab_tools.rs`. `validate_now::resolve_on_path` is `pub fn` in `src/ui/settings/validate_now.rs` (used by `check_binary`).
- **Acceptance:** Open Tools with no `weidu` / `mod_installer` on PATH → both rows show red `× not on $PATH` text below the input. Install `weidu` system-wide → row flips to `ok · <resolved path>` after the debounce settles.
- **SPEC:** §11.3.
...
- **What:** 2-col grid via `ui.columns(2, ...)` for an exact 50 / 50 split (matching the wireframe `gridTemplateColumns: "1fr 1fr"`). Each row uses an end-capped layout — label flex-left, optional unit hint, input / toggle flush-right — so longer hint copy on right-column toggles can't push controls past the column boundary. Left column "Timing & limits" renders six ValueRows per SPEC §11.5 (Custom scan depth, Mod install timeout, Auto-answer initial delay, Auto-answer post-send delay, Tick (dev), Prompt context lookback). Right column has two subsections: "Install behavior" (ToggleRows for Prompt sound cue, Download missing mods, Casefold filename matching) and "WeiDU command-line flags" (ToggleRows for `-a`, `-x`, `-o`).
- **Where:** New file. Row primitives (`end_capped_row`, `paint_input`, `paint_toggle`) live inline in `tab_advanced.rs` — the General-tab-shared `value_row` / `toggle_row` widgets aren't reused here because their internal layout doesn't right-align the control.
- **Acceptance:** Each ValueRow follows the absorb-the-gate pattern: an empty field means "use BIO default"; a filled field means "override". Internally, the renderer maps an empty input to setting both `<field>_enabled = false` and `<field> = default_value` in `Step1Settings`; a filled input maps to `<field>_enabled = true` and `<field> = parsed_value`. This preserves BIO's two-field representation without exposing the gate to the user. Resizing the window keeps the column boundary at the geometric midpoint; the toggle / input controls always end at the right column edge.
...
- **Corrupt-file policy (SPEC §13.14).** `bio_redesign_settings.json` holds reconstructable UI preferences, so it does **not** get the registry's terminal-error treatment. On a corrupt / unreadable file, `OrchestratorApp::new` renames it aside via `RedesignSettingsStore::backup_corrupt_file()` (→ `bio_redesign_settings.json.corrupt-<unix-ts>`), logs a warning, and continues with `RedesignSettings::default()`. The backup rename is what prevents the next debounced write from silently overwriting the bad file. `load()` stays pure.
- **Acceptance:** Editing the user name in NameRow persists across restarts. Switching theme persists. The file `bio_redesign_settings.json` appears in the platform config dir. A corrupt `bio_redesign_settings.json` at launch is renamed to `…corrupt-<ts>` (not overwritten), the app still launches with default preferences, and no terminal error pane is shown.
- **SPEC:** §11.1, §13.14.
