...
2. **`DestinationNotEmptyWarning` unreadable in Light mode.** `WARN_FILL` was a *premultiplied* (baked-dark) constant → an opaque dark box on the Light parchment. Fixed to un-premultiplied amber composited source-over the theme background (pale wash on Light, subtle on Dark); `const`→`fn` because `from_rgba_unmultiplied` is not `const fn` in this egui version. **Follow-up (same day):** QA showed the resulting amber surface is strong/theme-invariant and the `text-muted` grey was unreadable on it in both modes — so the box text now uses **dark ink (`#1a2638`)** per the §12.2 toned-surface rule (header solid, secondary line at ~70%), a deliberate deviation from the wireframe's `var(--text-muted)`, recorded in SPEC §4.1.
