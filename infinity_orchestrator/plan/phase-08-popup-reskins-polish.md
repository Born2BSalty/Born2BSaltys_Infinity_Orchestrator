...
- **Full light-mode color pass (deferred 2026-05-29, user).** The redesign was tuned dark-first; light mode reads off across the app in general — including the new soft overlay drop shadow (`redesign_overlay_shadow` light value in `src/ui/shared/redesign_visuals.rs`). Action: a dedicated light-mode audit + retune of the redesign token set (and the overlay shadow) so light theme reads as deliberately as dark. Not a per-token fix — a coherent pass.
- **Soft overlay shadow on the 4 custom-frame dialogs.** Confirm/Delete, Fork-info, Load-Draft, and Share-code set their own `.frame()`, which bypasses the global `window_shadow`, so they stay flat while every other overlay carries the soft dark-teal shadow. Action: add `.shadow(redesign_overlay_shadow(palette))` to each dialog frame (4 one-liners). Left out of the drop-shadow-pass PR (#28) per "ship now".
...
Rationale + the Fix-Run-5/6 history these refinements sit on top of: see [`decision-log.md`](decision-log.md) 2026-05-18 entries.
