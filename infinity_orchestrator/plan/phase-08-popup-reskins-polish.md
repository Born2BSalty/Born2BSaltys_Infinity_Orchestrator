...
## Deferred backlog — recorded from Phase-6 Fix-Run 2 (NOT yet task-scoped; do not implement here)

- **#7b — fork/import preview weidu lines = 3-hue (user-deferred to Phase 8, 2026-05-17).** Colors-only → fits the Phase-8 UI-colour reskin. It is the **shared** `src/ui/install/preview_tabs.rs` renderer (Install + fork preview); SPEC §6.7 is currently "Step 3 / Step 4 only" — Phase 8 reconciles §6.7's scope (extend to the preview weidu tabs, applied consistently incl. the Install preview) when it does the colour pass. No SPEC change now; this resolves the Fix-Run-2 SPEC CONFLICT (user final authority).
- **#4a — per-component prompt popup grows vertically (user-deferred to Phase 8, 2026-05-17).** Root cause = one line in **protected BIO** `src/ui/step2/prompt/prompt_popup_step2.rs:31` (`ui.set_min_size(ui.available_size())` self-feedback inside its `egui::Window`; aggregate branch :127 same line, out of scope). User direction: **Phase 8 likely introduces a NEW carve-out for important-but-small BIO bug fixes** — this is the first candidate (~1-line bounded-size change, Text branch). The carve-out is authored at Phase 8 (a SPEC §1 addition then), not now; this resolves the Fix-Run-2 PLAN GAP.

These three items surfaced during the Phase-6 user-feedback Fix-Run 2 (2026-05-17). They are **recorded here so they are not lost**, but they are **deliberately deferred** — none were in Fix-Run 2's scope, and they are not yet broken into Phase-8 tasks (P8.T*). A future planning pass promotes each into a concrete task before any implementation.

1. **BIO grey pane-border recolor (carve-out #6).** Some reused BIO sub-renderers still paint pane/group borders in BIO's grey rather than the redesign `border-strong` token, so a few inner panes read as a foreign grey against the redesign chrome. This is squarely the **carve-out #6** colour-only swap surface (state-aware `theme_global::*` → `redesign_*(palette)`), but the *exact* offending accessor sites have not been enumerated. Action when scoped: audit the reused-BIO render path for `theme_global` border accessors that the orchestrator actually shows, add them to the carve-out #6 table, swap value-only (no branch/logic change). **Concrete instance — Phase-6 verification #2 (user, 2026-05-18):** the redesign GameTab strip across Steps 2/3/4 reads as having a *grey bar beneath it*. Root cause is **not** the GameTab widget's own bottom border (that was removed in Phase 6, commits `ab4453b`/`fad78c3` — no bottom bar in any state, identical across all three steps) — it is the **reused BIO content pane immediately below the tab strip painting a grey top edge** against the redesign chrome. When this backlog item is scoped, the audit must specifically cover the BIO content-pane top/border accessor on the Step-2/3/4 reused render path directly under the tab strip, and value-swap it to the redesign border token.

2. **Window-narrow card-bottom misalignment.** At narrow window widths, the bottom edges of side-by-side cards/boxes (e.g. Home cards, the Create starting-point boxes) can fall out of vertical alignment when one column's content wraps to a different height. Cosmetic, width-dependent; not a correctness issue. Action when scoped: equalize row-of-cards heights (measure-tallest-then-set, or a shared min-height per row) so a wrapped column doesn't shorten its neighbor.

3. **App-wide "notifications disappear too fast" UX revamp.** The transient confirmations / toasts (`✓ saved!`, `✓ Copied import code …`, the Step-5 / Home toasts, etc.) auto-dismiss on a short fixed timer (~1.6–1.8s) with no dwell/hover-to-hold and no history. Users miss them. Action when scoped: a coherent app-wide notification model — longer/while-hovered dwell, optional manual dismiss, possibly a small recent-notifications surface — applied uniformly across Home / Workspace / Settings / Create / Install (this supersedes the per-site ad-hoc timers, including the Load-Draft copy confirmation and the Save-Draft flash).
