...
| `src/ui/step3/content_step3.rs` | #6 | `draw_tab_issue_badge` lines 52-62: `let (text_color, fill_color) = if has_blocking { (theme::conflict(), theme::conflict_fill()) } else { (theme::warning(), theme::warning_fill()) }` — two-arm conditional, each branch's two accessors swap for redesign counterparts. `render` line 249: unconditional `theme::warning()` for the compat-rules-error label. Both `draw_tab_issue_badge` and `render_toolbar` and `render` gain `palette: ThemePalette` arg; threaded from `page_step3::render` which itself gains `palette` (and is the entry point that the orchestrator's workspace step router calls). |
