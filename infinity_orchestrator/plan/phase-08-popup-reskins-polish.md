...
| `src/ui/step3/content_step3.rs` | #6 → **mostly SUPERSEDED by P6.T2d (2026-05-17)** | **Scope change:** the Step-3 C4 chrome (P6.T2d, SPEC §7.1) replaces BIO's `content_step3` toolbar/tab chrome with net-new redesign code — so `draw_tab_issue_badge`, `render_toolbar`, and `render`'s chrome path are **no longer reskinned here** (the orchestrator never calls `page_step3::render` once P6.T2d lands; the legacy `BIO` binary still uses them, but BIO is not the Phase-8 reskin target). Carve-out #6 for Step 3 reduces to the **reused reorder-list rows only** — i.e. the `src/ui/step3/list_rows_step3.rs` row below (lock-icon / compat / prompt pill colour sites in the drag list the C4 wrapper reuses read-only). If P6.T2d has **not** yet landed when Phase 8 runs, fall back to the original plan for this file (badge/toolbar/render `palette`-threaded token swaps); the Phase-8 run must check P6.T2d status first and annotate which path it took. |
...
### P8.T5 — Step 3 reorder list + toolbar (carve-out #6)

- **What:** Apply the carve-out #6 swaps for the Step 3 cluster. Files in scope:
  - `list_rows_step3.rs` — `if is_locked` branch + PROMPT pill unconditional
  - `content_step3.rs` — `if has_blocking` branch + compat-rules-error unconditional
  - `toolbar_compat_step2.rs` — `unwrap_or` fallback for compat color
- **Where:** Edit each file. Same diff bar as P8.T4.
- **Acceptance:** Open Step 3. Drag components to create / break conflicts. Verify: tab issue badges (conflict / warning tones) render redesign tokens; per-row compat pills render redesign tokens; lock-icon glyph renders `redesign_warning(palette)` when locked, `redesign_text_disabled(palette)` when unlocked. Light theme also produces correct values.
- **SPEC:** §7.
...
6. Open Step 3. Drag components. Verify: tab issue badges use redesign tokens for blocking vs non-blocking; per-row compat pills use redesign tokens; lock icon flips between `redesign_warning(palette)` and `redesign_text_disabled(palette)`.
