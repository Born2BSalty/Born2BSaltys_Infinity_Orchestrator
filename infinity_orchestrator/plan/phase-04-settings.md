...
- **What:** Four PathRows for `weidu.exe`, `mod_installer.exe`, `7z executable`, `Git executable`. Each row shows the detected version as a hint (e.g., `v249 ✓`, `system ✓`, `2.45.0 ✓`).
- **Where:** New file.
- **Acceptance:** Each row binds to its `Step1Settings` field. Version detection runs once per app launch (cached in `OrchestratorApp::tool_version_cache`) and is re-run on `Validate now`.
- **SPEC:** §11.3.

### P4.T5 — `tab_accounts` renderer

- **What:** Three cards (GitHub, Nexus Mods, Mega). Each card has a 32×32px square avatar with initials (`GH`, `NX`, `M`), the service name, a connection-state `Pill` (`connected` info-tone with `as <name>`, or `not connected` neutral), and a primary/secondary action button. **Uses `account_card::render` from `widgets/account_card.rs`** (the service card chassis defined in the New Files inventory above) — implementers should call the widget rather than re-implementing the card layout per service. GitHub's `connect`/`disconnect` opens the OAuth flow via `oauth_glue::start_github_flow(orchestrator)`. Nexus and Mega `connect` show a "not yet implemented" hint inline.
- **Where:** New file.
- **Acceptance:** Clicking GitHub `connect` triggers the OAuth device flow. The popup (rendered via `oauth_glue::render_github_popup_if_open` from `OrchestratorApp::update`, see P4.T9) opens with the verification URI + user code. Completing the device flow updates the card to `connected as <name>`. **Before implementation:** run `grep -n "^pub fn\|^pub(crate) fn\|^pub(super) fn" src/ui/step1/github_auth_popup_step1.rs src/core/app/app_step1_github_oauth.rs` to confirm the entry points are reachable. If everything is `pub` / `pub(crate)`, the orchestrator's `oauth_glue.rs` sibling calls the popup renderer and flow-runner directly. If a required entry turns out to be `pub(super)`-without-fallback, `oauth_glue.rs` instead drives the device flow against BIO's lower-level public HTTP / token-storage helpers (no carve-out needed; the sibling extends to whatever public surface is reachable).
