...
- **What:** `NavDestination::Workspace { modlist_id: Option<String> }` (defined in Phase 2 as nullable) now carries a required id once Create / Resume / Home Resume routes set it. `page_router::render` for `Workspace { Some(id) }`:
  ...
  4. **Per-frame Settings re-sync (per M2).** Call `workspace_state_loader::sync_paths_from_settings(&orchestrator.settings_store, &mut orchestrator.wizard_state)`. This copies the path fields (`bgee_game_folder`, `bg2ee_game_folder`, `iwdee_game_folder`, `mods_folder`, `mods_backup_folder`, `tools_folder`, `temp_folder`, plus the `weidu` / `mod_installer` / `7z` / `git` tool paths) from the current `Step1Settings` into `wizard_state.step1`. **This is run on every workspace render** — not just on workspace open — so any Settings → Paths edits made while the user was away from the workspace propagate to the active workspace's WizardState before the step renderer or install hook reads them. Cost is O(small number of `String::clone`s) per frame; negligible.
