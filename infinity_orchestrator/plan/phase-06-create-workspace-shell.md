...
- **What:** Dispatches the current `WorkspaceStep` to the appropriate renderer. **All dispatch happens at the router layer for consistency.** Step wrappers render the chrome but return the action up; the router dispatches every action via `step_action_dispatch::dispatch_stepN(action, orchestrator)`. The body looks like:
  ...
  ```
  Step 2 calls the orchestrator-side `workspace_step2::render` (the **Step-2 C4 treatment** — see P6.T2c): net-new redesign chrome (title, full-width `flex` search, the wireframe toolbar button set, **no** "Restart App With Diagnostics", Details pane hidden-by-default per SPEC §6) that reuses **only** BIO's component-tree + details sub-renderers (`bio::ui::step2::list_pane_step2::render_list_pane` / `details_pane_step2::render_pane`) in orchestrator-owned layout rects. BIO's `page_step2`/`frame_step2` are **not** called. Step 3 calls BIO's existing public `pub fn render` and ignores the `()` return (per H2) — the pre-P6.T2d interim; P6.T2d replaces this arm with `workspace_step3::render` (C4 chrome, its own run, same H2 `()`/dirty-bit mechanics). Step 4 calls the orchestrator-side `workspace_step4::render` (per C4 — see P6.T2b) and its returned `Option<Step4Action>` is dispatched uniformly via the router's `dispatch_step4` arm. **All dispatch happens at the router layer for consistency.** Step wrappers render the chrome but return the action up; the router dispatches.
...
### P6.T2d — Step 3 orchestrator-side chrome wrapper (C4-style — net-new chrome around BIO's reorder list) — ADDED 2026-05-17, NOT YET IMPLEMENTED

> **Status:** user-approved 2026-05-17 (closes a verified plan gap — Phase 6 originally specced Step 3 as a direct `page_step3::render` call; Phase 8 carve-out #6 is colour-only and structurally cannot restyle Step 3's chrome, so the wireframe's structurally-different Step 3 chrome had no owner — see overview.md 2026-05-17 entry + SPEC §7.1). **Implementation = its own dispatched `plan-implementer` run**, not done here. Until that run lands Step 3 still calls BIO's `page_step3::render` directly (the pre-P6.T2d state; P6.T2/H2 mechanics below stay valid for that interim).

- **What:** Build `src/ui/workspace/step3/workspace_step3.rs` — a net-new orchestrator-side Step 3 renderer, the direct analogue of P6.T2c (Step 2) / P6.T2b (Step 4). The wireframe's Step 3 chrome differs structurally from BIO's `content_step3` (SPEC §7.1: action-row count; the BGEE/BG2EE tab row + aggregate `N conflicts`/`N prompts` pills + `Undo`/`Redo`/`Collapse All`/`Expand All`; **no** `Export diagnostics` — curated like Step 2 dropped "Restart App With Diagnostics"). Carve-out #6 is colour-only and cannot restructure BIO's `content_step3::render`/`render_toolbar`; the CRITICAL DIRECTIVE forbids editing it — so the chrome is net-new and only BIO's reorder/drag **list** sub-renderer is reused.
  - **Signature:** `pub fn render(ui: &mut egui::Ui, orchestrator: &mut OrchestratorApp)` returning `()` (Step 3 has no action enum, per H2 — the reused list mutates `&mut WizardState` directly: drag-reorder, undo/redo, collapse, multi-select). BIO's `bio::ui::step3::page_step3::render` is **not** called.
  - **Net-new chrome (orchestrator layout rects, redesign tokens):** the action-row count "_N_ components ready to install on _\<tab\>_ · across _M_ mods"; the tab row — BGEE/BG2EE via the **shared `crate::ui::workspace::widgets::game_tab::game_tab`** (the ONE uniform GameTab, no bottom bar in any state — identical to Steps 2/4) + the aggregate conflict/prompt pills + `Undo`/`Redo`/`Collapse All`/`Expand All` redesign buttons wired to the same BIO entry points BIO's `render_toolbar` uses (read the source for the exact calls — undo/redo via `step3_history`, collapse/expand state). NO per-step hint here (the workspace shell renders it).
  - **Reused BIO sub-renderer (read-only — directive decision-order 1):** the reorder/drag list body only. Identify the narrowest BIO entry that renders the grouped drag list **without** BIO's toolbar/tabs (audit `src/ui/step3/`: `list_step3.rs` / `list_rows_step3.rs` / the drag machinery `state_drag_step3`/`service_drag_ops_step3`) and call it inside an orchestrator-owned, hard-clipped, fixed-size rect (the Step-2 `clipped_pane` precedent — the list never bleeds into the nav bar). If no sub-toolbar-free public entry exists, surface a `PLAN GAP` (do **not** edit BIO to carve one — escalate; same discipline as the Step-2 C2 audit).
  - **C4 boundary (verify like Step 2/4):** grep-prove `bio::ui::step3::page_step3::render` has **zero call sites** in the orchestrator after this lands (the legacy `BIO` binary still renders its own `page_step3`). Net-new files only; zero BIO source edits; BIO-source guard empty.
- **Where:** New files `src/ui/workspace/step3/mod.rs` + `workspace_step3.rs` (+ split helpers as needed). The router's Step 3 arm (P6.T2) switches from the direct `page_step3::render` call to `workspace_step3::render` when this run lands; the dispatch mechanics (no `Step3Action`, H2 `()` return, the dirty-bit fingerprint over `wizard_state.step3.<tab>_items`) are unchanged.
- **Acceptance:** Step 3 shows the net-new redesign chrome matching `screens.jsx` Step 3 (action-row count; shared GameTabs with no bottom bar; aggregate pills; Undo/Redo/Collapse/Expand; no Export-diagnostics) wrapping BIO's reused drag list with full functionality (drag-reorder, group collapse/expand, undo/redo, shift-click range select, conflict/prompt pills). The list never overlaps the workspace nav bar. Tabs render identically to Steps 2 & 4 (one shared widget).
- **SPEC:** §7 (esp. §7.1 C4 chrome decision), §7.2 (reuse BIO drag machinery), §1 (decision order; carve-out boundary; structural-difference rationale), §2.2.
