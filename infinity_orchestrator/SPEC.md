...
- A state-dependent action cluster on the right:
  - For an **in-progress** card: primary **`resume`** + Kebab with `Copy import code` / `Open data folder` / `Rename` / `Delete`.
  - For an **installed** card: **`open`** + Kebab with `Copy import code` / `Open install folder` / `Open data folder` / `Rename` / `Reinstall` / `Delete`. (The wireframe's `play` button is renamed `open` for v1 alpha — see [§3.2](#installed-modlist-card). `Open data folder` opens the per-modlist appdata directory `%APPDATA%\bio\modlists\<id>\` — distinct from `Open install folder`'s destination; see the Kebab tables in [§3.2](#in-progress-kebab-items).)
...
| Open data folder | Opens the modlist's per-modlist *appdata* directory `%APPDATA%\bio\modlists\<id>\` in the OS file manager — the directory that holds `workspace.json` and the per-install `-u` `weidu_component_logs/` ([§13.12](#1312-automatic-flag-policies) #2 / [§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)). Distinct from the installed card's `Open install folder` (which opens the destination). If the data directory does not exist yet (a brand-new build before its first persisted workspace write), surface an error in the standard bottom status / error area — do not create or open a missing folder (same posture as `Open install folder`). |
...
| Open data folder | Opens the modlist's per-modlist *appdata* directory `%APPDATA%\bio\modlists\<id>\` in the OS file manager — `workspace.json` + the per-install `-u` `weidu_component_logs/` ([§13.12](#1312-automatic-flag-policies) #2 / [§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)). Distinct from `Open install folder` (the destination); placed right after it. If the data directory is missing, surface an error in the standard bottom status / error area — do not create or open a missing folder. |
...
- `Box label="overall progress"` — "_N_ / _T_ mods · _P_%" big label + the overall progress bar. _P_% is **driven by BIO's real aggregate download signal** — the live "N/M archives downloaded" BIO emits (`"Downloading updates: N/M"`) blended with extract/stage completion — so the bar actually *climbs through the (longest) download phase* instead of resting on a flat per-phase constant (an earlier interim flat-phase-fraction average sat frozen at 45% for the entire download — fixed). Monotonic non-decreasing (every input — the aggregate, the past-download count, the staged count — is non-decreasing, so the bar never regresses; no 100→80 stall) and reaches 100 only when every mod is staged. _N_ counts mods whose archive has finished downloading (extracting or staged), so the count advances *through* the phases rather than staying `0 / T` until the last archive stages.
- `Box label="mod progress"` — a **4-column** grid (mod / source / status / **per-mod progress bar**) for every mod, **vertically scrollable inside a height-bounded area** (50+ mods reachable; the overall-progress Box stays fixed above; the scroll area is capped so the `Cancel` footer always stays on-screen). The **per-mod progress bar is a core requirement and IS shown**: its fill is *that mod's own* lifecycle position (queued empty → downloading → extracting → ✓ full), so it advances **individually per mod, separate from the overall bar** (mod A can be full while mod B is still queued). BIO exposes per-mod *completion* (downloaded/extracted membership), not a per-mod byte-%, so the downloading sub-phase is one determinate step rather than a continuous fill — each row still steps forward on its own schedule as its archive lands/extracts. Status text `queued` / `downloading` / `extracting...` / `✓ staged`; faint for queued, normal for active, success-green for done. (This reverses an earlier interim "status-only, no per-row bar" — a deliberate user-directed correction: an individual per-mod bar is required.)
...
- Generated **only** via `pack_meta` (see [Generation mechanism](#generation-mechanism-pack_meta)) — at Save Draft, install-start, and post-install regeneration. The four sibling keys are always present and consistent for a given modlist; only `allow_auto_install` differs by path (see below).
...
2. **`-u` (per-component WeiDU logging)** — always ON. On install start, the install runner creates a `weidu_component_logs/` folder and points `-u` at it. The user does not configure a path. Today's BIO requires this path to be set in Step 1; the redesign derives it. **The `-u` `weidu_component_logs/` folder lives in the per-modlist *appdata* directory `%APPDATA%\bio\modlists\<id>\weidu_component_logs` (the same per-modlist directory that already holds `workspace.json`), NOT in the user's destination folder.** WeiDU's `-u` backend rejects a log-folder path containing a space, and the destination folder is user-free-form (it may contain spaces); a space in the destination would defeat the `-u` preflight and stop the auto-build (a permanent inert 0/0). The per-modlist appdata directory is program-controlled and space-free, so the `-u` folder needs that no-space anchor while the per-install WeiDU-log *source* folders ([§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)) — a different code path, not subject to the no-space preflight — stay in the destination ([Appendix A.14](#a14-granular-weidu-log-mode-flags)).
...
**Per-install — derived (never asked). Created inside the modlist's destination folder, with one deliberate exception: the `-u` `weidu_component_logs/` folder lives in the per-modlist *appdata* directory (see its bullet — WeiDU's `-u` backend forbids spaces and destinations are user-free-form, so that one folder needs a no-space anchor).**
...
- **`-u` per-component WeiDU-log folder** — the `weidu_component_logs/` directory [§13.12](#1312-automatic-flag-policies) #2 points `-u` at. **Created in the per-modlist *appdata* directory `%APPDATA%\bio\modlists\<id>\weidu_component_logs` — the same per-modlist directory that already holds `workspace.json` — NOT in the destination.** Fixed name, not user-configurable. Rationale: WeiDU's `-u` backend rejects a log-folder path containing a space; the destination is user-free-form (may contain spaces) and a space there would defeat the `-u` preflight and stop the auto-build (a permanent inert 0/0), whereas the per-modlist appdata directory is program-controlled and space-free. It is resolved via the same registry/`WorkspaceStore` per-modlist directory resolution that yields the `workspace.json` parent (never a hand-joined `%APPDATA%` path), so the `-u` logs sit beside the modlist's reproducibility data. This is the **only** per-install directory not under the destination — premise-verified: WeiDU's no-space preflight is `weidu_log_folder`-only; the Mods folder, the WeiDU-log source folders, and the game-clone dirs are not space-restricted and all correctly stay in the destination.
- **WeiDU-log source folder(s)** — where the share-code importer writes the code's baked-in `weidu.log` and where the saved-log / auto-build flow reads it back (see the Pipeline-reuse contract below). Created per-install inside the destination with fixed names, derived (never asked); distinct from both the Mods folder and the `-u` per-component folder. Single-game uses one folder; **EET uses two** (its importer writes a BGEE-phase log *and* a BG2EE-phase log — two distinct folders so neither clobbers the other). This is part of the same derived per-install set BIO's install runner reads — without it the importer cannot write the imported log and the pipeline never starts.
