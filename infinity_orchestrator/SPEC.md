...
> **C4 chrome — Step 3 (decided 2026-05-17, user-approved).** Step 3's chrome **above the reorder box** — the action row (count) and the tab row (BGEE/BG2EE game tabs + aggregate pills + `Undo`/`Redo`/`Collapse All`/`Expand All`, and **no** `Export diagnostics`/`Restart App With Diagnostics`, exactly as Step 2's C4 curated its button set) — is **net-new orchestrator-side redesign chrome**, the direct analogue of the Step 2 (`frame_step2`) and Step 4 (double-`Save`) C4 treatments. BIO's `page_step3::render` is **not** called for the chrome; only BIO's **component-reorder/drag list sub-renderer is reused read-only** (the drag-and-drop machinery stays BIO's per [§7.2](#72-reorder-list)). Rationale is identical to Step 2's: the wireframe's Step 3 chrome is **structurally different** from BIO's `content_step3` toolbar, and the colour-only carve-out #6 *cannot restructure* it (no new/removed/reordered branches) — so the chrome must be net-new, not a BIO reskin. The per-step hint already renders in the workspace shell (`workspace_hint_line`), not here. The BGEE/BG2EE tabs render the **one shared `workspace::widgets::game_tab`** widget used identically by Steps 2/3/4 (a real tab — rounded-top, **no bottom border in any state**; never a per-step duplicate painter). Implementation is the plan's **P6.T2d**, dispatched as its own run; until that run lands Step 3 still renders BIO's `page_step3::render` directly (the pre-P6.T2d state).
