...
1. **Modlist name + Game** (split row).
   ...
   - Right (auto width): label "game". **What renders here depends on the selected starting point (see *Starting-point selection* below):**
     ...
     - **Import/paste box selected** → the dropdown is **replaced by a read-only contextual note** ("comes from the imported modlist"), faint, sketchy-bordered. On the import path the game is **derived from the pasted share code** (the modlist's game travels in the code — [§5.3](#53-fork-paste--fork-preview--fork-download) / [§5](#5-create--author-a-new-modlist) / [§13.3](#133-share-code-bio-modlist-v1)), so the user does not — and cannot — pick it here. Switching boxes does not lose the typed name / destination (the Setup Box is shared by both paths).
...
- A single primary **`Start →`** button at the **bottom-right**, styled **exactly like the workspace nav-bar forward (`Next →`) button** (it reuses that styling — one styling source). `Start →` dispatches per the selected box: from-scratch ⇒ `WorkspaceView` in `scratch` mode; import ⇒ `fork-paste`.
- The selected box also drives the **Game** slot of the Setup Box (ComboBox for from-scratch; the read-only "comes from the imported modlist" note for import — see the *Modlist name + Game* row above).
