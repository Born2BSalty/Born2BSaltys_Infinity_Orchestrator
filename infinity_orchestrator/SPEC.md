...
>
> 16. **Per-modlist download-source & installed-refs resolution (USER-AUTHORIZED 2026-06-08)** — per-modlist ownership of mod download-source pins (`mod_downloads_user.toml`) and the installed-refs record (`mod_installed_refs.toml`), fixing the cross-modlist version bleed. Authorized in full by `infinity_orchestrator/SPEC-per-modlist-download-sources.md` §9 (eight functional surfaces) and detailed file-by-file in `infinity_orchestrator/plan/phase-08-per-modlist-download-sources.md`. **Every edit below is additive and inert on the unset-ambient / no-override path** — the legacy `BIO_legacy` binary never sets the ambient, so its read/write/export/import is byte-identical to today. Authorized BIO files and edit shapes:
