...
> 13. **Step-2 components-pane box chrome (USER-AUTHORIZED 2026-05-29)** — config-level chrome edits in `src/ui/step2/list_pane_step2.rs` only. No logic change. The `Frame::group` inner margin is `{left: 12, right: 0, top: 8, bottom: 0}` — right margin is zero so the scroll region reaches the box’s inner-right border and the floating scrollbar (painted at `bar_outer_margin = 2` from the region edge) hugs the border with the standard ~2 px gap. Content `min_size` is computed as `left_rect.size() − inner_margin_totals − 2 × border_stroke_width` so the group’s outer rect fits within `left_rect` and all four border strokes render inside the clip — horizontal total is `left + right = 12`, vertical total is `top + bottom = 8`. A `SCROLLBAR_RESERVE = 14.0` constant (bar_width 12 + bar_outer_margin 2) is subtracted from the scroll content’s usable width via `ui.set_max_width` so mod rows stay clear of the floating bar without reserving a solid lane. `ScrollStyle::floating()` is used to match Step 3, with identical `bar_width = 12.0`, `bar_inner_margin = 0.0`, `bar_outer_margin = 2.0` values.
>
