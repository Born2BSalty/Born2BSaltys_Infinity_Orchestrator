...
- Below that is a bottom nav row with `← Previous` / `Next →` buttons and a forward hint (`next: <label>`, or `final step` on Step 5). **The wireframe's `on <Step N> · <Label> · step <i> of <total>` step-indicator label between `← Previous` and the forward hint is intentionally NOT rendered — a deliberate user-directed wireframe deviation (recorded so a review does not restore it).** The progress bar above already shows the current step and its number, so the indicator was redundant chrome; only the forward `next:`/`final step` hint is kept (it is the lone forward-looking affordance, not the removed indicator). On the **first** workspace step (Step 2) `← Previous` returns to **Home** — the user reached the workspace via a Home `resume`/`open`, so first-step Previous closes that loop rather than being a dead control. Intentional affordance-forward deviation from the wireframe's former first-step *disabled* state (recorded; overview 2026-05-16). `← Previous` is force-disabled only by the Phase-7 install-running gate.
...
1. **Destination folder** — `FolderInput` with `browse` button. If the destination is non-empty, a `DestinationNotEmptyWarning` Box appears with toggle-style choice buttons (per the wireframe `screens.jsx:123-154`, **not** native radio widgets — the active choice is the `primary`-styled button): **Clear contents** / **Backup contents then proceed** / **Continue partial installation** (the last only when partial is allowed). Choice → flag mapping per [§13.12](#1312-automatic-flag-policies) #1/#6. Continue-partial mode skips the share-code requirement entirely. **A valid destination is required to proceed:** the footer primary stays disabled until the destination resolves to a **real existing folder on disk** — a typed value must be an actual directory (arbitrary text does not satisfy it; `browse` always yields a valid one). This applies in every mode (partial necessarily has one already). The `DestinationNotEmptyWarning` amber fill composites over the app's dark-ish backdrop to a **dark olive surface in both themes**, so its text uses **white ink, theme-invariant** (header solid, the secondary line at reduced alpha). Note this is *not* the [§12.2](#122-pill-tones) dark-on-tone rule — that applies to the *light* pill tones; this composited surface is dark, so it inverts to light ink. Deliberate, recorded deviation from the wireframe's `var(--text-muted)`, which is illegible on this surface in both Light and Dark.
...
- **Subline** — `by @<author> · review what will be installed before BIO downloads anything` when the code carries `author`; if `author` is absent, drop the `by @<author> · ` segment and show just `review what will be installed before BIO downloads anything`. Never invent an author. *(The wireframe's `FORK_META` always carries an author for demo purposes; the absent-author rendering specified here is **SPEC-authoritative** — it is conditional behavior, not a copy string the wireframe overrides. Phase 5 is consume-only, so the absent path is what actually renders until generation ships.)*
...
- `Box label="overall progress"` — a **phase indicator + two labelled progress bars**, one per phase. The pipeline has **two distinct ordered phases** (`InstallPhase`: `Downloading` → `Extracting`); the box presents them as a smooth `0 → 100` Downloading bar followed by a fresh `0 → 100` Extracting bar (the phase indicator names the active one; the active bar uses the live accent, the other is muted; after hand-off the Download bar stays full-but-muted):
  - **Download overall = `Σ downloaded_bytes ÷ Σ expected_bytes`** across all wanted archives (per-archive `expected_bytes` = the live `Content-Length` else the baked `expected_size` from [§13.3 Archive meta](#archive-meta-name-size-hash)). It is **not** an `N / M` count and **not** a blended-aggregate heuristic.
  - **Extract overall = a separate `extracted ÷ to-fetch`** that starts at `0` when the Extract phase begins and **never inherits the Download bar's value** — the two never share a bar.
  - **[§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives) checksum-then-skip mods** render first as **"✓ already downloaded"**, count as instantly-complete in the **Download** denominator, and are **excluded from the Extract** denominator.
- `Box label="mod progress"` — a **4-column** grid (mod / source / status / **per-mod progress bar**) for every mod, **vertically scrollable inside a height-bounded area** (50+ mods reachable; the overall-progress Box stays fixed above; the scroll area is capped so the `Cancel` footer always stays on-screen). The **per-mod progress bar is a core requirement and IS shown**, and it carries a **real per-mod byte fraction** during the download phase, **read every frame**: the redesign's own net-new parallel streaming downloader (`install_runtime::stream_downloader`, [§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)) reads each response's `Content-Length` and accumulates bytes, so a downloading row's fill is its *own* `bytes / size` (size = the live `Content-Length` else the baked `expected_size`) — it advances **individually per mod, separate from the overall bars** (mod A can be byte-progressing while mod B is still queued). An honest marquee is shown **only** when there is no determinate size at all (never a fake %). The grid repaints continuously while the pipeline is active and is idle when it is not (`step2_needs_repaint` is true while `modlist_auto_build_active`). Post-download (extracting → ✓ staged) the bar is the row's own lifecycle phase; a checksum-then-skip mod renders as **"✓ already downloaded"**. Status text `queued` / `downloading` / `extracting...` / `✓ staged`; faint for queued, normal for active, success-green for done.
- The content-addressed archive staging (hashing/scanning archives) runs **off the per-frame render path** — one-shot at each state transition (pre-download placement once, post-download content-addressing once), never re-hashing archives every frame (which froze the UI during download/extract). Dedupe / coexist / lock semantics are unchanged; only the scheduling.
- **Known Phase-8 chassis item (not a §4.3 behavior gap):** at narrow widths (~1045 px) the source / status columns of the mod-progress grid collide — a Phase-5-chassis column-width responsiveness nit (app min width is 1024). The live data and the two-phase model above are correct; the column-width fix is a Phase-8 item, unchanged.
...
After a successful install, this screen renders the **full [§9.2](#92-post-install-layout-installcompletetrue) post-install layout — including the success banner** (the green `Installed` pill + `<N> mods · <C> components · no errors` + `ran <MM:SS> · finished <relative>` row, exactly as the workspace path renders it). The §9.2-vs-§9.3 split is by **install state** (finished vs. during), **not** by entry point — the post-success layout is entry-point-agnostic. The **only** §4.4-specific deviation from §9.2 is that **the Share import code button is omitted**: the code that produced this install is the one the user already pasted, so it is not re-offered from this entry point. Concretely, the post-install action row gains **`Return to Home`** + **`Open install folder`** ([§9.2](#92-post-install-layout-installcompletetrue), Appendix B.2), sitting below the success banner.
...
#### Starting-point selection (two **selectable boxes** — user-directed deviation)

> **User-directed UX deviation (final authority), supersedes the wireframe's two cards-with-sub-buttons.** The canonical wireframe had two starting-point **cards**, each with its own in-box primary CTA (`start →` / `paste share code →`). The user directed a different interaction (recorded here as intentional so a review does not restore the cards):

- A **`Choose one`** header above the two boxes.
- Two **selectable boxes**, side-by-side. **Clicking anywhere on a box selects it** (the selected box draws an **accent** border + a faint accent tint; the other draws the normal `border-strong`). The in-box `start →` / `paste share code →` sub-buttons are **removed**. Box 1 = **New modlist from downloaded mods** ("Scan your local mods folder, pick components, reorder, then install. Starts from an empty selection."). Box 2 = **Import and modify another modlist** ("Paste a share code. BIO downloads the mods, preselects components, applies the order, then drops you on Step 2 to review and adjust."). The from-scratch box is selected by default.
...
> **Symbol-glyph (cmap-verified).** The only `→` on this screen is on `Start →`; it is rendered via the nav-bar forward button (`firacode_nerd` arrow + `poppins_medium` prose, side by side) because `→` U+2192 is absent from the Latin-only Poppins subset (would tofu to `?`) and present in the bundled full FiraCode Nerd build. No raw `→` is passed to a Poppins-only button.
...
- **`Delete` is fully wired (user-directed deviation, final authority).** The canonical wireframe left the LoadDraft Kebab `Delete` inert (`onClick: () => {}`); the user directed it to **fully work via the same machinery the Home kebab uses** (recorded here as intentional). Choosing `Delete` arms the danger [`ConfirmDialog`](#107-confirmdialog) (`Delete "<name>"?`, the wireframe-verbatim body incl. the install-folder path); on **Confirm** it runs the shared `registry::operations::delete_modlist` — the registry entry is removed and the on-disk install folder is removed behind the same safety guard Home uses; the card disappears from the list (and from Home) immediately. On Cancel nothing changes. This is the **exact Home delete path reused, not reimplemented**.
...
- **Action cluster** (right side, in the same row, growing to fill space):
  ...
  - **Kebab** (`···`) — menu items, in this order: `Show Details panel` / `Hide Details panel` (persistent toggle — the verb reflects open/closed state; intentional, *not* the wireframe's former `✓ ` prefix: the bundled Poppins subset lacks `✓` U+2713 and the verb is clearer — rationale in overview 2026-05-16; do not re-flag), `Clear All`, `Select Visible`, `Collapse All`, `Expand All`, `Jump to Selected`. The toggle pins the right-side Details pane open across row selections; the per-row hover `[?]` button still works alongside it ([§6.8](#68-details-panel)).
...
> **Step-3 chrome is net-new orchestrator code (the C4 treatment) — the direct analogue of Step 2 ([§6](#6-step-2--scan-and-select)) and Step 4 ([§8.1](#81-layout)).** The wireframe's Step-3 top area (`screens.jsx::ComponentsPanel`) is structurally different from today's BIO Step-3 chrome (BIO renders a heading + hint, a raw tab/badge row, and a dev/diagnostics button). Colour-only carve-out #6 cannot restructure BIO's `content_step3` toolbar and the CRITICAL DIRECTIVE forbids editing it, so the workspace renders **net-new redesign chrome** (`src/ui/workspace/step3/`) and **reuses only BIO's drag-reorder list body read-only** ([§7.2](#72-reorder-list)). The orchestrator's workspace step router calls `bio::ui::workspace::step3::workspace_step3::render`; BIO's `page_step3` / `content_step3` / `render_toolbar` / `frame_step3` are **not** called (the legacy `BIO` binary still renders its own Step 3 — unaffected). Like Step 2's C4 dropped its dev button, Step 3's C4 **omits** `Export diagnostics` / `Restart App With Diagnostics` (curated, not in the wireframe).
...
- **Both lines are rendered for Step 3** (the earlier "shell-only, no duplicate — the body never repeats the shell hint" rule was wrong for Step 3 and is corrected: the two strings are *different* lines, the body line is the wireframe `ComponentsPanel` line, and the canonical wireframe does draw it; the user is final authority).
- **The Step-3 "_N_ components ready to install on _\<tab\>_ · across _M_ mods" count line is REMOVED — it is Step-4-only.** The canonical wireframe `ComponentsPanel` never drew this line (only Step-4's `OrderPanel` does — [§8.1](#81-layout)). An earlier revision of this section specified it for Step 3 too (spec-over-wireframe); that drift is **reverted** here and the user is final authority. The count still renders in Step 4 ([§8.1](#81-layout)). (No `Save weidu.log's` button on Step 3 either — that also lives only in Step 4.)
...
- Payload includes everything today's BIO writes — `format_version`, `bio_version`, `game_install` (BGEE/BG2EE/EET/IWDEE), `install_mode`, `weidu_logs` per game, `source_overrides` (a `mod_downloads_user.toml` excerpt), `installed_refs` (pinned tp2 → source + commit/tag), `mod_configs` (pre-supplied config files) — **plus five schema-additive sibling keys the orchestrator adds**: `allow_auto_install: bool` (see [below](#allow_auto_install-schema-additive-field-default-true)), the **provenance trio** `name`, `author`, `forked_from` (see [Provenance](#provenance-name-author-forked_from) below), and **`archive_meta`** — the per-archive `{name,size,hash}` Wabbajack-compile record (see [Archive meta](#archive-meta-name-size-hash) below).
...
**Install-Modlist-paste / Reinstall persist the code the orchestrator already has — they do *not* regenerate via `pack_meta`.** On the Install-Modlist-paste pipeline (and Reinstall, which routes through it) `state.step3` is **empty** at install-start — the share-code import's `reset_workflow_keep_step1` clears it and the async scan→apply-log has not run — so `export_modlist_share_code` (hence `pack_meta`) cannot produce a code there. The orchestrator instead **persists the code it already holds** (the user-pasted code, or for Reinstall the entry's stored `latest_share_code`), changing **only the `allow_auto_install` bit** on its decoded payload via the net-new `share_export::set_allow_auto_install` (the `pack_meta` envelope **minus** the `export_modlist_share_code` step — it decodes the held code to an opaque `serde_json::Value`, sets just that one key, and re-encodes with the same byte-format BIO's decoder round-trips). Every other key — including the pasted code's baked-in `name` / `author` / `forked_from` — rides through **verbatim** (this is how the provenance the recipient pasted is preserved; the honest `Shared modlist` fallback is *not* triggered, because the real code is not rewritten). `set_allow_auto_install` is independent of `state.step3`; it composes — never patches — BIO (it does not call `export_modlist_share_code` at all). A non-decodable held code is persisted **verbatim** (the real code is the priority over the false→true draft nicety).

**The legacy BIO Step 5 "Export Modlist…" path is intentionally untouched** — it has no orchestrator name/author/registry concept and produces a field-less code (consumed via the defaults, exactly as today).
...
**Rationale for carve-out #5 (vs. a wrapping struct in the orchestrator):** A wrapping struct would mean two parallel serde types — BIO's `ModlistSharePayload` and an orchestrator-side wrapper that mirrors all its fields plus the four BIO-decoded keys — and, worse, a parallel orchestrator-side decoder/preview, because BIO's `preview_modlist_share_code` would no longer see the wrapped code. The maintenance burden compounds every time a future field is added to BIO; both the wrapper and the parallel decoder drift and need manual reconciliation. The carve-out lets the four BIO-decoded keys live in the one canonical struct (`#[serde(default = "default_true")]` for `allow_auto_install`, `#[serde(default)]` for the provenance trio) so a single decode/preview path serves Install and Create. The consume-side addition is mechanical (four fields + one default function + four propagation lines in `share_preview()`); the generate side adds **zero** BIO code (the `pack_meta` envelope sibling composes `export_modlist_share_code` and round-trips an opaque `serde_json::Value`). The fifth sibling, `archive_meta`, is deliberately **outside** the carve-out — it is orchestrator-owned (`decode_archive_meta`, never a `ModlistSharePayload` field), so BIO needs no change for it at all. Zero behavior change for existing flows: every code BIO has ever produced is treated identically because the keys default to `true` / `None` / empty / omitted.
...
*(The prior `stage_known_archives`-only behavior — reusing only prior-attempt-locked archives of the **same** modlist — was necessary but insufficient: it never recognized an archive already present by content on a **first** install / reproduce-from-code. Checksum-then-skip is the realization of the skip-if-present-by-content intent.)*
...
Every install — regardless of entry point (**Install Modlist** paste flow, **Create → New modlist from downloaded mods**, **Create → Import and modify**, **Load Draft**) — captures a **BIO-MODLIST-V1 import code** at the **start of the install** and writes it to the modlist's destination folder as a clearly-named text file. The default filename is `modlist-import-code.txt`. **Source of the code by entry point** ([§13.3](#133-share-code-bio-modlist-v1)): the **Workspace / Create / Save-Draft** paths generate it from internal state via `pack_meta`; the **Install-Modlist-paste / Reinstall** paths persist the code the orchestrator already holds (the pasted code, or the Reinstall entry's stored `latest_share_code`) with only the `allow_auto_install` bit set (`share_export::set_allow_auto_install` — `state.step3` is empty there, so `pack_meta`/`export_modlist_share_code` cannot run; the pasted code's baked-in provenance rides through verbatim). Pasting an Install-Modlist share code also **registers an in-progress `ModlistEntry`** at install start ([§13.1](#131-modlist-persistence)) — the modlist appears on Home as in-progress until a clean exit flips it to installed; a Reinstall reuses its existing entry (no second registration) and its `Installed → InProgress` flip happens at the Install-click ([§3.1](#cards-in-the-filtered-list)).

At install start the code is written with **`allow_auto_install = false`** ([§13.3](#133-share-code-bio-modlist-v1)) — the install has not yet succeeded. (On the Workspace path that is the `pack_meta`-built code with the bit `false`; on the Install-Modlist-paste / Reinstall path it is the held pasted/stored code with only its bit set to `false` via `set_allow_auto_install`.) Any `archive_meta` ([§13.3 Archive meta](#archive-meta-name-size-hash)) the source code already carried rides through this install-start code **verbatim**, exactly as provenance does (it is not recomputed here — the verified record is baked at the clean-exit transition below). The same code is stored as the registry entry's `latest_share_code`, and the registry entry's `install_started_at` is stamped (every variant, every attempt). The file is written upfront — before WeiDU runs — so the artifact survives even if the install crashes, gets cancelled, or finishes with errors; while an install is in-flight (or after a crash/cancel/errored exit) the on-disk file is that conservative-by-default `allow_auto_install = false` draft (the "I crashed, here's what I was trying" artifact).

On a **clean successful exit**, the registry's `latest_share_code` is set to the `allow_auto_install = true` code **and the on-disk `modlist-import-code.txt` is rewritten with that same `allow_auto_install = true` code** (the same destination + filename it was written to at install start). (On the Workspace path the true-bit code is `pack_meta`-regenerated; on the Install-Modlist-paste / Reinstall path it is the held install-start code with only its bit flipped to `true` via `set_allow_auto_install` — so install-start and clean-exit differ only by that one bit plus the freshly-baked `archive_meta`, the pasted code's provenance preserved throughout.) The same `flip_to_installed` transition bakes the verified per-archive **`archive_meta`** (`{name,size,hash}` from the per-install lock — [§13.3 Archive meta](#archive-meta-name-size-hash)) into this clean-exit code, so the post-install re-shared code carries the exact archive set the install consumed (schema-additive; empty ⇒ omitted ⇒ backward-compatible). Once the install succeeds, the file sitting next to the game install is the verified, auto-install-eligible code — forwarding it to a collaborator recreates a **directly-installable** modlist — and it always matches the registry's `latest_share_code` (the Share import code dialog reads the registry value; the two cannot disagree post-success). Only the import-code file is rewritten: no registry snapshot is ever written to disk — the live registry remains the single registry view (H8). The post-success rewrite is part of the same single post-install transition that flips the registry state ([§9.2](#92-post-install-layout-installcompletetrue)); a missing/empty destination or a write failure is non-fatal (logged and skipped — the registry's `latest_share_code` is canonical regardless). A **non-clean exit** (cancel / nonzero / failure) leaves the install-start `allow_auto_install = false` draft on disk untouched — the rewrite lives exclusively inside the C3-gated post-success transition.
