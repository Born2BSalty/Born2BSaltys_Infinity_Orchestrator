...
**BIO-fidelity.** Reuse `default_mod_downloads.toml` + `user_mod_downloads.toml` and the existing fetch/selection logic. Source editor popup → Appendix A.5. Per-modlist user overlay per [§13.12b](#1312b-per-modlist-data-ownership); the default manifest stays global.

### 13.5 Compatibility rules

**BIO-fidelity.** Reuse `default_step2_compat_rules.toml` + `step2_compat_rules_user.toml` and the compat engine (`src/core/app/compat/`). Step 2 UI ([§6](#6-step-2--scan-and-select)) consumes the unified output. Per-modlist user additions / overrides per [§13.12b](#1312b-per-modlist-data-ownership); the default rule set stays global.
...
Auto-answer prompt memory ([§13.6](#136-auto-answer-prompt-memory)) is excluded from per-modlist scope — `@wlb-inputs` scripted tokens are the supported path.
