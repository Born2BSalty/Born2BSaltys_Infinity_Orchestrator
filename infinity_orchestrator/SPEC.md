...
>
> 22. **Updates-popup post-extract result clear (USER-AUTHORIZED 2026-06-11)** — fixes the Updates popup's post-extract "Extracted" confirmation list (and its extract-failed companion) lingering after a subsequent Check Updates. `clear_full_update_preview_results` (runs on every full update check via `apply_full_update_preview_state`) cleared the downloaded / download-failed result lists but omitted the two extract result lists, while its sibling clear `invalidate_update_selected_results` (the source-editor-save path) already cleared all four — so a fresh full check left the stale Extracted note showing. Authorized BIO file and edit shape:
>   - `src/core/app/step2/update/app_step2_update_preview.rs` — `clear_full_update_preview_results` gains two `.clear()` calls for `update_selected_extracted_sources` and `update_selected_extract_failed_sources`, mirroring `invalidate_update_selected_results` so the two clear paths cover the same result set; plus a `#[cfg(test)]` regression test pinning the clear. **The scan-complete path is deliberately not touched:** an extract triggers an immediate auto-rescan (`orchestrator_app.rs`) that must preserve the just-set Extracted note so the user can read it, so clearing on rescan would erase the confirmation — the note is meant to survive the rescan and clear at the next full check, which this fix delivers. No signature, struct, field, or other behavior change.
>
> **Standing allowances (USER-AUTHORIZED 2026-06-11; (d) added 2026-06-12; (e) added 2026-06-12) — five behavior-neutral edit classes, permitted on any existing BIO source going forward without a per-file carve-out.** These ease the directive for low-risk maintenance; they do **not** authorize any functional or behavioral change, which still requires a numbered carve-out above. The test for all five is identical: **no change to behavior, logic, data flow, or control flow.** When in doubt, treat the edit as functional and escalate for a numbered carve-out.
...
>   - **(d) Comment hygiene.** Comments and rustdoc may be edited or removed on BIO source: deleting verbose, redundant, or paper-trail comments (references to plan tasks, phases, runs, SPEC sections, carve-out numbers, handoff/decision-log docs, forensic change-history narration, or coding-agent artifacts) and condensing the rest to concise plain-language descriptions of what the code does and how it fits the surrounding flow. Comments compile out, so this is behavior-neutral by construction. The SPDX-License-Identifier / Copyright header lines must remain. Code, identifiers, and string literals are out of scope; `cargo fmt --all` stays banned (scope edits to specific files).
>   - **(e) Code formatting.** Existing BIO source may be brought to **`cargo fmt`** conformance — whitespace, line-wrapping, and layout only. Behavior-neutral by construction (no token, identifier, or string-literal value changes). `cargo fmt` is the authoritative formatter; scoped per-file `rustfmt` can diverge from it on style-edition details (the source of past grandfathered churn), so conformance is judged by `cargo fmt --all -- --check`. A **deliberate, separately-reviewed normalization pass may run `cargo fmt`** over the repo; the (a)/(d) ban on the blanket command stands only for routine mixed-content commits, which keep formatting scoped to the files they already change.
>
