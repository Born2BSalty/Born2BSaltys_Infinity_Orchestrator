...
2. **`-u` (per-component WeiDU logging)** — always ON. On install start, the install runner creates a `weidu_component_logs/` folder **inside the modlist's destination folder** and points `-u` at it. The user does not configure a path. Today's BIO requires this path to be set in Step 1; the redesign derives it. This is part of the same per-install, inside-the-destination, derived directory set as the Mods folder and the WeiDU-log source folders ([§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)), with a fixed name the user cannot override ([Appendix A.14](#a14-granular-weidu-log-mode-flags)).
