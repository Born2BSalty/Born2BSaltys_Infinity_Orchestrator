...
- `Box label="mod progress"` — a **4-column** grid (mod / source / status / **per-mod progress bar**) for every mod, **vertically scrollable inside a height-bounded area** (50+ mods reachable; the overall-progress Box stays fixed above; the scroll area is capped so the `Cancel` footer always stays on-screen). The **per-mod progress bar is a core requirement and IS shown**, and it carries a **real per-mod byte fraction** during the download phase: the redesign's own net-new parallel streaming downloader (`install_runtime::stream_downloader`, [§13.12a](#1312a-directory-architecture-global-vs-per-install--content-addressed-archives)) reads each response's `Content-Length` and accumulates bytes, so a downloading row's fill is its *own* `bytes / Content-Length` — it advances **individually per mod, separate from the overall bar** (mod A can be byte-progressing while mod B is still queued). When the server sends **no `Content-Length`** the row shows an honest active-but-unmeasured fill (byte-count progress with an indeterminate total — **graceful**, never a fake %). Post-download (extracting → ✓ staged) the bar is the row's own lifecycle phase. Status text `queued` / `downloading` / `extracting...` / `✓ staged`; faint for queued, normal for active, success-green for done. **This reverses the earlier interim rationale that "BIO exposes only per-mod completion, not a per-mod byte-%, so the download sub-phase is one determinate step"** — that was true of BIO's serial worker, which the redesign no longer uses for the download; the net-new streamer exposes a real byte signal, so the per-mod bar is a continuous fill during download (no-`Content-Length` → graceful indeterminate).
