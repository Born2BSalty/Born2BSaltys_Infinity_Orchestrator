...
- `Box label="overall progress"` — "_N_ / _T_ mods · _P_%" big label + a single progress bar. This is the **only** progress bar. _P_% is computed from a **monotonic non-decreasing per-phase weighting** (queued ≤ downloading ≤ extracting ≤ staged) so the overall bar can never regress as a mod advances through its phases (it does not stall or drop — e.g. no 100→80 regression). _N_ counts mods whose archive has finished downloading (extracting or staged), so the count advances *through* the phases rather than staying `0 / T` until the last archive stages.
- `Box label="mod progress"` — a **3-column** grid (mod / source / status) for every mod in the modlist, **vertically scrollable** (50+ mods are all reachable; the overall-progress Box stays fixed above it). Per-row **status only** (no per-row progress bar / no per-row %): `queued`, `downloading`, `extracting...`, `✓ staged`. Colors: faint for queued, normal for active, success-green for done. BIO emits only an aggregate download percentage (no per-asset %); a fabricated per-row % is therefore **not** shown (it would render every in-flight row with an identical, meaningless number) — the single honest overall bar above is the only percentage.
