...
> **⚠ PHASE 7 NOT YET COMPLETE — one premise-checked acceptance gap remains (a final Fix-Run, in flight).** Run-4b's premise-check confirmed (against real source) that `modlist-import-code.txt` is written **only** inside `on_install_start`, which the **Install-Modlist-paste / Create-import / Load-Draft / Reinstall** pipeline flows **bypass** (Run-4a trigger-split). So plan **P7.T11 / SPEC §13.13 / Verification-#5** ("the file appears for every install path incl. Install-Modlist-paste + Reinstall") is **not met for those entry points** — a real gap no single run's brief covered. The **final P7 Fix-Run** closes it (invoke the §13.13 install-start bundle at the `page_install.rs` Preview→Downloading Install-click for the pipeline entry points). **Do not run the Phase-7 end-pass until the orchestrator signals the Fix-Run is also landed + the end-of-arc no-op gate is green.**
