...
- **Directory architecture + content-addressed archives (SPEC §13.12a).** Global, Settings-defined (§11.2): **Mods archive** (ALL downloads for ALL modlists, always), **Mods backup**, **Game sources**. Per-install, inside the destination: the **Mods extract/stage/scan folder** (removed on clean success), the **WeiDU-log SOURCE folders** (`<dest>/weidu_log_source/{bgee,bg2ee}`), + the **game-install clone dirs** (already specced by §13.12 #3/#4 — always cloned, fixed names; the redesign never surfaces BIO's no-clone path, BIO untouched). **Exception (user-approved Fix A, 2026-05-18 — the ONLY per-install dir not under the destination):** the `-u` `weidu_component_logs/` dir lives in the per-modlist *appdata* dir `%APPDATA%\bio\modlists\<id>\weidu_component_logs` (resolved via the canonical `registry::store_workspace::modlist_data_dir` — the same resolver as `workspace.json`'s parent; never hand-join `%APPDATA%`). WeiDU's `-u` backend forbids spaces, the destination is user-free-form (a space ⇒ a permanent inert 0/0), so the `-u` dir needs that no-space program-controlled anchor. Premise-verified: the no-space preflight is `weidu_log_folder`-only — every other per-install dir correctly stays in the destination. SPEC §13.12 #2 + §13.12a carry the full record; `per_install_dirs::{resolve,derive_per_install_dirs}` thread a `modlist_id` (pure `_with_data_dir` cores keep tests DATA-LOSS-safe). The global Mods-archive is **content-addressed**: hash-on-write, same name+hash ⇒ cross-modlist dedupe, same name/different hash ⇒ both coexist, the modlist lock records its hash, extract selects the matching archive per install. This is a **net-new orchestrator staging layer wrapping** `app_step2_update_download`/`app_step2_update_extract` (zero BIO modification). The orchestrator drives BIO's `import_modlist_share_code` → saved-log/auto-build pipeline; global paths reach the owned `WizardState` via `sync_paths_from_settings` (the Install screen does NOT collect game paths). Lands in **Phase 7 P7.T17** (the pipeline terminates in the install runtime); Phase 5 shipped the §4.3 chassis only. Rationale: overview.md 2026-05-16 revision log.
