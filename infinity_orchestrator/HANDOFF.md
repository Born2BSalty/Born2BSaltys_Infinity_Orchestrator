...
| 7 | Step 5 install runtime + Reinstall + install concurrency + rail-nav lock | ✓ **COMPLETE** via PR #5 + PR #6. Open follow-up: Item #4 Create-fork post-route UX on branch `xgatt/create-then-modify-fix` — Runs 1 + 2 landed; Run 3 (rescan-wipe + Step-4→5 weidu auto-save + gate-eval log) uncommitted, awaiting user re-test. |
...
**Open follow-up** — Item #4 Create-fork post-route UX (branch `xgatt/create-then-modify-fix`, single-branch arc). **Run 1** `2c95df9`: fork-route un-blocked. **Run 2** `b284525`: apply-imported-WeiDU-log selection on Workspace landing + suppress auto-opened Update Check popup. **Run 3 (uncommitted)**: three bugs surfaced in user testing — (i) rescan-wipes-step2: `should_reset_completed_install_route`'s `has_route_context() + clean_exit` proxies fired on a fork-then-modify Workspace because `destination`/`import_code` legitimately stay populated and `last_exit_code` could carry from a prior install; replaced with a new `PostInstallResetGate` enum flag (set in `maybe_flip_to_installed_on_clean_exit` after `flip_to_installed` succeeds, cleared in `reset_completed_install_runtime`); (ii) Step-4 → Step-5 nav edge now calls `auto_save_step4_weidu_logs` so the imported on-disk weidu.log files reflect Step 2/3 edits before the install runner reads them (mirrors legacy BIO's `NeedStep4SaveThenAdvance`); (iii) `fork_extract_complete` gate-eval log tightened to also require `!scan_running && !apply_pending && !update_preview_pending` so the diagnostic only emits at the actual terminal evaluation, not throughout the post-extract wait window. ZERO BIO source touched. Auto-update / asset-substitution concern still deferred — Born2BSalty's thread per user. Awaiting commit + user re-test.
