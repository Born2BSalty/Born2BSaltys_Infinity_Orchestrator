...
**Open follow-up** — Item #4 Create-fork post-route UX (branch `xgatt/create-then-modify-fix`, single-branch arc). **Run 1** `2c95df9`: fork-route un-blocked. **Run 2** `b284525`: apply-imported-WeiDU-log selection on Workspace landing + suppress auto-opened Update Check popup. **Run 3** `b67695b`: rescan-wipes-step2 via new `PostInstallResetGate` enum flag, Step 4 → Step 5 auto-save weidu logs, gate-eval log spam tightened. **Run 4** `57bc5c4`: suppress an auto-build install regression unmasked by Run 3 — `arm_auto_build` leaves the auto-build flags true through the fork pipeline; `advance_pending_saved_log_flow` was firing `start_auto_build_install` once apply settled. Fix clears both flags in `stage_fork_download::render_live` as soon as extract is done. **Run 5 (uncommitted)**: two cascading bugs Run 3 + 4 surfaced — (i) Workspace install's success banner + Step-5 console were wiped right after install completion because Run 3's new `PostInstallResetGate` flag fired the reset for both Install-Modlist paste AND Workspace installs. The OLD predicate's `has_route_context()` check effectively suppressed the reset for Workspace installs because `reset_install_screen_to_paste` had been called during the fork-route, but the flag-based replacement fires for both. Fix: gate the flag-set on `!from_workspace` so the reset only arms for Install-Modlist paste / Reinstall. (ii) `auto_save_step4_weidu_logs` (BIO) early-returns when `install_mode == install_exactly_from_weidu_logs`, which is what the parent share code's mode propagates to forks; the user's fork-then-modify edits never reached disk. Fix: a redesign-side `write_step4_weidu_logs_unconditional` writes the files via the public `build_weidu_export_lines` regardless of install_mode. ZERO BIO source touched. Auto-update / asset-substitution concern still deferred — Born2BSalty's thread per user. Awaiting commit + user re-test.
