...
- **Non-Latin symbol glyphs must be rendered via `firacode_nerd`, not Poppins.** The shipped Poppins TTFs are a Latin-only subset, so symbol/dingbat glyphs (`∞` U+221E, `✓` U+2713, etc.) tofu when drawn in any `poppins_*` family — and a tofu'd `✓` silently becomes `?`, which has masqueraded as real state (a *detected* game looked *missing*). This bit us twice (brand mark, then `game_installs_detected`). Convention: render only the symbol glyph in `firacode_nerd` (full coverage) and keep adjacent prose in Poppins. Note the Latin-Extended Poppins swap above will **not** fix this — `∞`/`✓` aren't Latin-Extended either; `firacode_nerd` is the permanent answer for symbols.
...
- **`ModlistSharePreview` needs `allow_auto_install` added too** (per M-new-2), not just `ModlistSharePayload`. Phase 5 P5.T10 documents the two-surface addition under carve-out #5.
