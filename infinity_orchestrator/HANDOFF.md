...
| 7 | Step 5 install runtime + Reinstall + install concurrency + rail-nav lock | ✓ **COMPLETE** via PR #5 + PR #6. Open follow-up: Item #4 Create-fork post-route UX on branch `xgatt/create-then-modify-fix` — Runs 1-5 landed (verifiably-fixed list below). Two 🔴 data-loss bugs still open: **R1** double `prepare_destination` wipes freshly-extracted Mods/ at Step-5 Install click; **wrong-write-path** `sync_paths_from_settings` clobbers per-install fields with stale settings values so weidu.log writes to the previous modlist's folder. Full risk map in `.claude/INSTALL_WORKFLOWS_TRACE.md` (13 risks numbered R1-R13). |
...
**Open follow-up** — Item #4 Create-fork post-route UX (branch `xgatt/create-then-modify-fix`, single-branch arc). Five runs landed; two 🔴 data-loss bugs surfaced by the workflow-trace audit still need fixing before the arc closes.

**Verifiably fixed (Runs 1-5, all on origin):**
...
**Still open — 🔴 data-loss tier:**
- **R1: double `prepare_destination` on Backup/Clear** — `fork_pipeline_arm::mint_and_arm` runs `prepare_destination` once, then `consume_pending_destination_prep` at Step-5 Install click runs it AGAIN against the same `pending_destination_prep` persisted to `workspace.json`. On Backup: freshly-extracted Mods/ + weidu_log_source/ + modlist-import-code.txt all get archived a second time into a fresh `_bio_backup_<name>_<ts>`. On Clear: `fs::remove_dir_all` permanently destroys those files (no Recycle Bin). This is what nuked the user's WSETUP.DEBUG mid-install. **One-line fix candidate:** gate `consume_pending_destination_prep` on `!install_screen_state.pipeline_flags.armed()` so the workspace-step5 consumer becomes a no-op when the fork arm already ran.
- **Wrong write path** — `sync_paths_from_settings` (called from `populate_wizard_state_from_workspace` on Workspace cold-load) copies per-install path fields like `eet_bg2ee_log_folder` from `settings.step1` → `wizard_state.step1`, overwriting the correct per-install paths set by `derive_per_install_dirs` during the fork pipeline. The user's weidu.log write ends up at a stale previous-modlist destination (e.g., `C:\Games\BIO\installations\simpletest\weidu_log_source\bg2ee\weidu.log` instead of the current fork's `fork test 5\...`). **~4-line fix candidate:** call `derive_per_install_dirs(&mut wizard_state.step1, &entry.destination_folder, entry.game)` after `sync_paths_from_settings` in `populate_wizard_state_from_workspace`, so the per-install fields are re-set to the modlist's correct destination-based paths.

**Also still open — 🔴 Linux-only:**
