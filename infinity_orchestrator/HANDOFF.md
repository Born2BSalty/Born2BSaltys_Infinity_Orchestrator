...
- WorkspaceView shell hosting Steps 2–4 (Step 5 stubbed; Phase 7 wires it):
  ...
  - 4-step progress bar.
  - Per-step hint line.
  - Step body — **Step 2 is an orchestrator-side C4 chrome wrapper** (`workspace_step2::render`, P6.T2c — net-new redesign chrome (`step2_tab_row` + `step2_search`): title, full-width `flex` search + Rescan (disabled pre-Phase-7 per §13.12a) / Cancel-while-scanning, redesign GameTabs + Select-via-Log + Updates + clickable compat/prompt pills + count + Kebab[Show/Hide Details · Clear All · Select Visible · Collapse All · Expand All · Jump to Selected], **no** "Restart App With Diagnostics", Details pane hidden-by-default per SPEC §6) that reuses **only** BIO's tree / details / compat / prompt / updates sub-renderers + the same `pub(crate)` toolbar-action helpers `render_controls` itself calls (read-only); BIO's `render_controls` / `render_tabs` / `render_header` / `page_step2` / `frame_step2` are **not** called (the 2026-05-16 SPEC-CONFLICT resolution — the wireframe's Step 2 is structurally different from BIO's and colour-only carve-out #6 cannot restructure `frame_step2`). Step 3 currently renders via a direct call to BIO's existing `pub fn page_step3::render` with the orchestrator's owned `WizardState` — the **pre-P6.T2d interim**. **P6.T2d (approved 2026-05-17) gives Step 3 the same C4 chrome treatment as Step 2/4** (net-new redesign action-row + tab row + Undo/Redo/Collapse/Expand, reusing **only** BIO's reorder/drag list; closes a verified plan gap where the wireframe's structurally-different Step 3 chrome had no owner — Phase-8 carve-out #6 is colour-only and cannot restructure it). Implementation is its own dispatched run (not yet done). Step 4 is an orchestrator-side renderer (per C4 — replaces BIO's `page_step4::render` to avoid the double Save button). **The BGEE/BG2EE tabs in Steps 2/3/4 all render ONE shared widget — `crate::ui::workspace::widgets::game_tab::game_tab` — a real tab with NO bottom border in any state** (replaced the two former per-step duplicate painters whose all-four-sides stroke left a bottom bar under idle tabs; 2026-05-17).
...
- **OPEN SPEC CONFLICT — Step-4 WeiDU-line is 2 perceptual colours, not 3 (the Run-2b #3 fix, NOT resolved — awaiting a user decision).** `weidu_line.rs::build_weidu_job`'s parse is **structurally correct** (premise-checked: it produces 3 distinct token-coloured runs — path / `#id` / comment — for every canonical input shape, verified by the passing `three_colour_split_uses_redesign_tokens` test). But the plan-pinned token mapping (phase-06 `weidu_line.rs` file-inventory row: `<component_id>` → `redesign_text_muted`, `<component_label>` → `redesign_text_primary`) maps the numbers + comment to **adjacent neutral greys** (Dark `#A7B3BD` vs `#E6EDF3`; Light `#5c6a7a` vs `#1a2638`) — so they read as one colour and the list looks 2-tone. The canonical wireframe `WeiduLine` (`screens.jsx:794-802`, authoritative on visual detail per SPEC fidelity + spec-authority priority) and SPEC §6.7 both specify **three high-contrast hues**: path amber, `#0 #id` **dark-blue `#2f6fb7`**, comment **green `var(--success)`**. The plan's neutral-grey mapping is the drift from the wireframe/§6.7. `weidu_line.rs` was **NOT modified** — resolving this changes plan/SPEC-pinned visual behaviour and needs the user's call: (a) amend the plan `weidu_line.rs` row + SPEC §6.7's redesign-token mapping so `<component_id>` uses a distinct hue (a blue token; none exists in the redesign palette today) and `<component_label>` uses `redesign_success` (the wireframe's `var(--success)`), restoring 3 distinct colours; or (b) accept the 2-tone result against the wireframe/§6.7. See the Run-2b report's `SPEC CONFLICT` section.
...
- **C4 — Step 4 double Save button.** BIO's `page_step4::content_step4.rs` already renders a `Save weidu.log's` button; the wireframe places one in a new top action row. Two save buttons would render. Resolution: Phase 6 replaces BIO's Step 4 body with an orchestrator-side renderer (still calls BIO's save action via `bio::app::app_step4_*` public dispatch — only the rendering is net-new). **Extended 2026-05-17 to Step 2 and Step 3.** The same structural-difference logic applies to all three: the wireframe's Step-2 (`frame_step2`), Step-4 (double-Save), and Step-3 (`content_step3` toolbar/tabs) chrome differ structurally from BIO's, and the colour-only carve-out #6 cannot restructure them. Step 2 = P6.T2c (shipped), Step 4 = P6.T2b (shipped), **Step 3 = P6.T2d (approved, its own future dispatched run — closes the verified plan gap that Phase 8 carve-out #6 could not).** All three reuse only BIO's interaction sub-renderers (tree/details, save action, reorder/drag list) and render the **one shared `workspace::widgets::game_tab`** for the BGEE/BG2EE tabs (no per-step duplicate; no bottom bar in any state).
