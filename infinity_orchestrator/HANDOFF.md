...
| 7 | Step 5 install runtime + Reinstall + install concurrency + rail-nav lock | ✓ **COMPLETE** via PR #5 + PR #6. Open follow-up: Item #4 Create-fork post-route UX on branch `xgatt/create-then-modify-fix` — Runs 1-4 landed; Run 4 suppresses an auto-build install regression so the fork-import lands on Workspace for review instead of auto-installing through to Step 5. |
...
**Open follow-up** — Item #4 Create-fork post-route UX (branch `xgatt/create-then-modify-fix`, single-branch arc). **Run 1** `2c95df9`: fork-route un-blocked. **Run 2** `b284525`: apply-imported-WeiDU-log selection on Workspace landing + suppress auto-opened Update Check popup. **Run 3** `b67695b`: rescan-wipes-step2 via new `PostInstallResetGate` enum flag, Step 4 → Step 5 auto-save weidu logs, gate-eval log spam tightened. **Run 4 (uncommitted)**: suppress an auto-build install regression unmasked by Run 3 — `arm_auto_build` leaves `modlist_auto_build_active` + `modlist_auto_build_waiting_for_install` true throughout the fork pipeline; `advance_pending_saved_log_flow` was firing `start_auto_build_install` once apply + update_preview settled (in the same frame the predicate fired), which set `start_install_requested = true` and auto-installed instead of letting the user land on Workspace. Fix clears both flags in `stage_fork_download::render_live` as soon as extract is done (gated on `armed && archives_ingested && !extract_running`), so the fork path no longer triggers the auto-build install. Install-Modlist paste's auto-install behavior is unaffected (it goes through `stage_downloading::render`, not the fork render_live). ZERO BIO source touched. Auto-update / asset-substitution concern still deferred — Born2BSalty's thread per user. Awaiting commit + user re-test.
