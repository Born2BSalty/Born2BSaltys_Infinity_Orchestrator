...
- **IWDEE game-folder row** in Settings → Paths is bound to a placeholder `Step1Settings` field (`eet_bgee_game_folder`). `Step1Settings` doesn't have a dedicated `iwdee_game_folder` slot. When IWDEE is used in earnest, the schema needs the slot — escalation per the CRITICAL DIRECTIVE.
- **7z + git binary rows** in Settings → Tools are visual stubs — `Step1Settings` doesn't have backing fields for them. Same escalation path.
