...
| 7 | Step 5 install runtime + Reinstall + import-code auto-write + install concurrency + rail-nav lock | ✓ **COMPLETE** — shipped via PR #5 (merged `3d8287d`) + PR #6 (merged `6ea7b5c`). Open follow-up: Item #4 Create-fork extract / post-route UX — Run 1 (page_router `active_install_modlist_id` clear) landed `2c95df9`; Run 2 (post-route: components-not-selected, popup auto-opens) on branch `xgatt/create-then-modify-fix`, awaiting user re-test + commit. |
...
**Open follow-up** — Item #4 Create-fork post-route UX fixes (branch `xgatt/create-then-modify-fix`, off `main`, branch covers the whole arc — not per-run). **Run 1 landed `2c95df9`**: `page_router::clear_pending_reinstall_on_nav_away_from_install` was clearing `active_install_modlist_id` while the fork pipeline was mid-extract on the Create stage, so `fork_extract_complete_route_to_workspace` had no modlist id to route to. Fixed by skipping the clear during `CreateStage::ForkDownload`. **Run 2 implemented (uncommitted)**: post-route UX gaps surfaced by the user — (a) Workspace landed at Step 2 with `(0/3)` everywhere instead of the imported WeiDU log's selection; (b) Update Check popup auto-opened. Three changes, ZERO BIO source touched: (i) `drain_extract_parallel::handle_extract_finished` re-arms `pending_saved_log_apply = true` before kicking the post-extract scan so `advance_pending_saved_log_flow` writes the imported log's selection onto the populated step2; (ii) `stage_fork_download::fork_extract_complete` now also requires `!is_scanning && !pending_saved_log_apply && !pending_saved_log_update_preview` so the route doesn't fire mid-apply (the wizard_state would be wiped by the Workspace transition before apply runs); (iii) `stage_fork_download::render_live`'s fork-complete block clears the three update-popup flags + persists the workspace state with `scratch_mods_folder = wizard_state.step1.mods_folder` and the post-apply `order_bgee/bg2ee`; (iv) `step2_resume_scan::maybe_trigger_resume_scan` extended to fire in non-dev sessions when `scratch_mods_folder` is set (prefers it over `dev_scanned_mods_folder`). 10 new unit tests (3 fork-extract gate, 5 pick_resume_folder, 2 drain_extract re-arm). Auto-update / asset-substitution concern (user's intent #3) NOT addressed — investigation showed `app_step2_update_check` honours pinned `installed_refs` via `source_ref_matches`, so the substitution risk is theoretical when refs are pinned; suppressing the check entirely would empty the asset list (preview_update_selected IS the source-config→asset resolver per SPEC §13.12a Pipeline-reuse contract) and breaks downloads. Flagged for user decision as a separate item. Awaiting user manual-test on the new build.
