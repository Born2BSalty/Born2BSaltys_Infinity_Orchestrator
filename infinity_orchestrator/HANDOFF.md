...
- **Carve-out #5 now carries the provenance trio, not just `allow_auto_install`** (user-directed spec change, 2026-05-15 — overview.md revision log). The Phase-5 Run-4 BIO touch on `modlist_share.rs` is exactly: `allow_auto_install` + `name`/`author`/`forked_from` (`#[serde(default)]`) on `ModlistSharePayload` **and** `ModlistSharePreview`, a `default_true()` fn, a `ForkAncestor` struct, and 4 `share_preview()` propagation lines — nothing else. SPEC §1 "Modlist-share provenance application" is the exact authorized surface; the BIO-source guard must still find this the *only* BIO edit in Phase 5.
