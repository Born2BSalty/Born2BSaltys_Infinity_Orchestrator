...
| 6 | Create screen + Workspace shell (Steps 2–4) | ✓ **COMPLETE** — 4-run slice: **R1 done** (workspace spine + Step-2 C4 chrome, hardened 1b–1e), **R2 done** (Step-4 C4 renderer + workspace header/rename + save-draft + game tabs), **Step-3 C4 chrome done** (P6.T2d — `src/ui/workspace/step3/`, net-new chrome wrapping BIO's reused drag-reorder list; SPEC §7.1 / phase-06 P6.T2d / phase-08 P8.T5 cascade landed with it; overview 2026-05-17 §4); **R3 done** (Create screen `stage_choose` + Load Draft dialog + Create/Resume routing — P6.T7/T9/T13/T14; the `create_modlist` 5-arg PLAN-GAP + the SPEC §10.2 file-picker doc-drift were premise-checked and fixed in the same commit; overview 2026-05-17 §5); **R4 done** (P6.T8 fork sub-flow + lineage append + P6.T11 dirty-bit workspace persistence + P6.T15 nav-away flush; overview 2026-05-17 §6) — **Phase 6 COMPLETE; next is Phase 7 (install runtime).** **Fix-Run-3 (`0b9d53d`) + Fix-Run-4 (`28d9975`)** closed a workspace.json order-wipe DATA-LOSS regression in the Phase-6 persistence path. Fix-Run-3 (per-tab `extract` guard) was INCOMPLETE — it missed the one-frame-scan window where the scanned set lands non-empty but the cold-resume restore was skipped (user testing exposed it). Fix-Run-4 completes it: arm the scan-completion edge so the restore reconciles + a `restore_pending` early-return so no save path writes while a cold-resume restore is pending. Verified 285/0 + sentinel-clean (incl. every seed workspace.json). The deselect-last/single-component edit-loss is a SEPARATE pre-existing limitation, DEFERRED (minimal-fix boundary). **Phase-6 verification round:** 6 items user-confirmed closed; the Create screen (#4) reopened → **Fix-Run 5 (`600dbb3`)** — Create-screen wireframe-alignment cluster (one shared input chassis/height + right-edge alignment, equal-height Choose boxes, `Start →` in the shared dashed-border footer, import note → "imported") + #8 forked share code written to `%APPDATA%\bio\minted_fork_share_code.txt`; verified 287/0, SPEC §5.1 doc-synced. Fix-Run-5 shipped visibly-broken (right margin gone, footer not pinned) → **verification-process fix: `egui_kittest` headless rendered-snapshot gate** (`7086149`, `tests/ui_snapshot_create.rs`; standing rule — the orchestrator personally views full-shell multi-width PNGs before any UI handoff) → **Fix-Run 6 (`e305f92`)**: footer truly bottom-pinned, ComboBox-width right-margin-collapse fixed, equal input/game/browse geometry, selected-box text legible — **orchestrator-verified by direct render review at 1280/1045/1021/960 × scratch/import** (every user-reported #4 bug visually fixed); 289/0; no SPEC change (code now matches the §5.1 already synced). #8 forked share code user-confirmed end-to-end. Orphaned `forward_primary_button` dead-code + gate→golden-baseline promotion deferred. Awaiting user live visual sign-off on the gate-fresh binary. Detail: overview.md 2026-05-18 revision log |
