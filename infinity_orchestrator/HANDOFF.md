...
| 7 | Step 5 install runtime + Reinstall + install concurrency + rail-nav lock | ✓ **COMPLETE** via PR #5 + PR #6. Item #4 Create-fork post-route UX **arc closed — PR #8 merged (`xgatt/create-then-modify-fix`)** — Runs 1-6 landed; both 🔴 data-loss bugs closed (R1 double `prepare_destination` + wrong-write-path `sync_paths_from_settings` clobber). Remaining deferred items: **R7** Linux symlink follow on `delete_modlist`; **R6** Continue-partial-install dead UI; **Gap #4** silent skip in `build_extract_jobs`. Full risk map in `.claude/INSTALL_WORKFLOWS_TRACE.md` (13 risks numbered R1-R13). |
| 8 | Popup reskins + state-aware theme reads across BIO surfaces + polish + per-modlist data ownership refactor (P8.T14, SPEC §13.12b) | in progress — **P8.T2 + P8.T7 + P8.T9** (popup collapse chevrons via carve-out #2; anchor-on-collapse empirically branch (a) — no wrapper; dotted radial background painter + `redesign_dot(palette)` accessor + `shell_chrome` integration) landed via PR #13 (merged). **P8.T15 Step-3 list-body sibling delivered** (branch `feat/step3-list-body-sibling`, 10 commits, 2026-05-27 → 2026-05-28) — wireframe-fidelity dotted-border header bar with `redesign_rail_bg` fill, mod version label via `weidu_version::parse_version`, Nerd Font PUA padlock glyphs, chain icon + `≡` drag handle dropped, full-width drop-marker on drag; added **carve-out #11** (text-prompt jump-to-target Step-3 routing) and **carve-out #12** (non-uniform-pitch-aware drag target). See `plan/phase-08-popup-reskins-polish.md` §P8.T15 — Delivered for the full deviation list. **P8.T6 Step-5 sub-renderer color swaps delivered** (branch `feat/step5-color-swaps-p8t6`, PR #22 merged, 2026-05-27 → 2026-05-28) — original carve-out #6 swaps across `status_console_step5`, `content_install_row_step5`, `content_cancel_step5`, `content_dev_header_step5`, `status_phase_step5` + orchestration shims; plus follow-ons within the same branch (color retunes for Install-in-progress/Running/Finished/Waiting Input, rail active-item pill_text); plus carve-out #9 widened twice to add `content_install_row_step5.rs` + `menus_step5.rs` + `prompts/prompt_answers_step5.rs` (pattern (g) for `menu_button → redesign_btn + popup_below_widget`); plus ✓ Installed success state, workspace-nav final-step Next removal, "Open install folder" → game subfolder, action-row small-button polish + dropdown min-width. See `plan/phase-08-popup-reskins-polish.md` §P8.T6 — Delivered. Verified status (2026-05-28, code + git): **done** — P8.T1 token foundation (PR #14), P8.T4 Step-2 tree/Details (PR #14), P8.T2/T7/T9 popup-collapse + anchor + dot-background (PR #13), P8.T6 Step-5 (PR #22), P8.T15 Step-3 list-body sibling (PR #24). **Superseded** — P8.T5: the orchestrator renders the net-new `step3_list_body` sibling, not BIO `list_rows_step3`, so the in-place swap is moot. **Effectively done** — P8.T8: #2/#3/#4 shipped in Phase-7 P7.T17, #6 via `destination_prep`, #7 (`-autolog`/`-logapp`/`-log-extern`) ON by `Step1State` defaults (the orchestrator never disables them). **No work needed** — P8.T16: Step 4 already matches the wireframe. **P8.T10 app-wide notification framework delivered** (branch `feat/toast-notification-framework`, 2026-05-30) — `egui-toast 0.17.0` dep; `NotificationManager` at `src/ui/orchestrator/widgets/notification.rs`; wired to `OrchestratorApp`; 7 push sites migrated; `src/ui/home/toast.rs` deleted; bell history toggle in statusbar; SPEC §10.8 rewritten. 596 lib tests passing; both binaries gate-fresh; clippy pedantic+nursery 0 warnings. **Genuinely remaining:** P8.T3 (residual carve-out #1 WeiDU-line / prompt-popup token swaps in `format_step2` / `format_step3` / `prompt_popup_step2` — cosmetic, light-theme), P8.T11 (hover sweep — mostly egui-default today), P8.T12 (no unified clipboard helper — copy works via inline paths), P8.T13 (final smoke pass), P8.T14 (per-modlist data-ownership refactor per SPEC §13.12b — not started; the biggest remaining chunk). |
...
## Recent ships + process / tooling changes (2026-05-20 → 2026-05-29)

**2026-05-30 session — P8.T10 notification framework:**
- **P8.T10 delivered** — app-wide notification stack (`egui-toast 0.17.0`, `NotificationManager`). Bottom-right anchor, 4 severities (success/info/warning/error), error persists, others auto-dismiss with hover-pause, read-only last-5 bell-toggle history in statusbar. 7 push sites migrated; `home/toast.rs` retired. SPEC §10.8 rewritten; plan §P8.T10 marked Delivered. 596 lib tests; both binaries gate-fresh; clippy 0 warnings.

**2026-05-29 session — UI polish + drop-shadow pass + P8.T10 design:**
- **PR #27 (merged)** — Step-2/3 components-box chrome + scrollbar hugs the box's inner-right edge (row content reserved clear) + kebab sizing (height-matched to sibling buttons; right-aligned dropdown; 180px-capped menu) + Step-2 bottom-border fix. Carve-outs **#13** (`list_pane_step2.rs` box chrome) + **#14** (`details_paths_step2.rs` value `.truncate()`) added to SPEC §1.
- **PR #28 (merged)** — drop-shadow pass. Removed the fat solid-offset dark-teal shadow from progress bar / fork pill / account avatar + a per-button `BtnOpts.no_shadow` opt-out on the Home "paste import code" + "resume" buttons. Added a soft blurred dark-teal **overlay** shadow on all floating overlays via `redesign_overlay_shadow(palette)` + re-enabled `window_shadow`/`popup_shadow` (zero BIO source — BIO popups inherit it through egui visuals). SPEC §12.3 now states the two-style shadow doctrine.
- **PR #29 (open)** — plan deferred-backlog: full **light-mode color pass** (light mode reads off app-wide; dark-first design) + the **4 custom-frame dialog shadows** (Confirm/Fork-info/Load-Draft/Share-code bypass `window_shadow`). Both deferred per user.
- **Status corrections:** **P8.T11 effectively done** (button press-down in `redesign_btn`; `[?]` reveal via `row_action_visible`; tree/pill hover via egui defaults; in-BIO pill drop-shadow explicitly not required). **P8.T10 fully scoped + designed** — plan §P8.T10, `egui-toast = "=0.17.0"` compat-verified against egui 0.31.1; this is the next implementation chunk.

**Code:**
- **Carve-out #9 popup chrome — title bars + global title-overlay disable + forks sizing fix** (branch `feat/p8-popup-titles`, 2026-05-27) — single plan-implementer run + one orchestrator-inline follow-up fix bundled in one PR, ZERO BIO source outside the carve-out #9 popup family files. **Title styling:** new `src/ui/orchestrator/widgets/window_title.rs` exposes `redesign_window_title(palette, title) -> egui::WidgetText` (Poppins-Medium 18 px, `redesign_text_primary(palette)`); re-exported via `widgets/mod.rs`. Wired at five named popup `Window::new` sites: `update_check_popup_step2::render_main_popup` + `render_latest_fallback_confirm` + `render_forks_popup`, `update_check_popup_source_editor_step2::render_source_editor_popup`, `step1::github_auth_popup_step1::render`. **Global title-overlay disable:** new `v.window_highlight_topmost = false` in `src/ui/shared/redesign_visuals.rs::build_for` — closes the grey title-bar fill that egui paints at `egui-0.31.1/src/containers/window.rs:611–624` as a separate `header_color` rect (the fill defaults to `widgets.open.weak_bg_fill` and the redesign never sets that field, so it inherited egui's default grey). With the overlay off, every `Window` in the app now inherits `window_fill = redesign_shell_bg(palette)` on its title bar — risk surface accepted by the user; other popups visually verified clean. **Title alignment:** stays centered (egui hardcodes it at `window.rs:1152–1157` + `1226–1228`; no `Window::title_align()` API; user accepted the tradeoff to keep chevron + close X). **Forks-popup growth fix:** pre-existing bug from PR #15 the user surfaced during this run's test — `render_forks_popup` had no `min/max_*` clamps on its `Window` builder + the inner `auto_shrink([false, false])` `ScrollArea` inside a `Frame::group` had no `max_height`, so each input event re-measured ScrollArea + Resize widget grew + next event grew more (continuous on mouse-move). Fix mirrors the established `prompt_popup_step2`/`source_editor` precedent: `ui.set_min_size(ui.available_size())` at body entry; conservative `content_height = (avail - 48).max(80)` ScrollArea max-height; `auto_shrink([false, true])` (fill width, shrink-to-content vertically); `set_min_width` on the scroll content; Window gains `min_width(360) / min_height(220) / max_width(screen-40) / max_height(screen-80)` matching `render_main_popup`'s `9195988` shape. **Render-gate test:** `tests/ui_snapshot_p8_visuals_and_shadows.rs::render_popup_demo` now uses `redesign_window_title` so the regenerated PNGs (`target/render_gate/p8_visuals_and_shadows/popup_demo__{dark,light}.png`) demo the new title styling. **Verification:** `cargo test --lib --release` **567/0** (+2 from 565 baseline — both new `*_visuals_window_highlight_topmost_disabled` palette tests); whole-codebase clippy pedantic+nursery exits 0; both binaries no-op rebuild gate-fresh; BIO-source guard hits 3 files all inside carve-out #9 (the popup family); DATA-LOSS sentinel `feb421de…` `modlists.json` unchanged + 4 per-modlist `workspace.json` (1 delta on `KSE0T6WG0093` traced to legitimate user app activity between sessions, byte-stable across all test runs). Carve-out #9's scope is now: primary buttons + secondary buttons + section-header chrome (PR #15) + popup title bars + window title-overlay (this PR) + forks-popup sizing (this PR). **SPEC formalization of #9 still deferred** — the directive in §1 lists #1–#8; the precedent shipped by PRs continues to widen #9, doc-sync to land in a dedicated docs PR.

- **Phase 8 — popup collapse chevrons + dotted radial background** (branch `feat/popup-collapse-and-radial-bg`, PR #13 open, commit `49c3392`, 2026-05-23) — three bundled Phase 8 tasks in one plan-implementer run, ZERO BIO source outside the explicit 8 carve-out #2 chevron flips + 1 carve-out #3 companion line. **P8.T2** — 8 single-keyword `.collapsible(false)` → `.collapsible(true)` flips across 6 BIO popup files (compat_window, prompt_popup main + toolbar, update_check_popup main + confirm-latest-fallback + forks, source_editor, github_auth, content_cancel_step5); line numbers re-verified on HEAD `0a97cf3` pre-dispatch (plan doc's `update_check_popup_step2` numbers had drifted to `:122/:840/:879`). **P8.T7** — empirically verified branch (a): egui's native `Window` stores top-left `Pos2`; on collapse the window shrinks from the bottom and the title bar stays put; no anchor wrapper needed; `popup_collapse_anchor.rs` is NOT created. **P8.T9** — new `src/ui/shared/redesign_dot_background.rs` paints 1px-radius circles on a 20px grid via `std::iter::successors` (no f32↔int casts); new `dot_bg: Color32` field on `PaletteValues` + LIGHT premultiplied `(2,3,4,20)` / DARK premultiplied `(12,12,12,13)` values (math equivalent to wireframe `rgba(26,38,56,0.08)` / `rgba(230,237,243,0.05)`; the brief's `from_rgba_unmultiplied` API isn't `const fn`, so the agent used `from_rgba_premultiplied` with the equivalent values) + `pub const fn redesign_dot(palette)` accessor + unit test; `shell_chrome::render_shell` paints the pattern at `LayerId::background()` covering `ctx.screen_rect()` before any `TopBottomPanel`/`CentralPanel` renders. `cargo test --lib` **519/0** (+1 new `dot_bg_differs_between_palettes`); whole-codebase clippy pedantic+nursery exits 0 (orchestrator caught one `clippy::map_unwrap_or` in the new test file the agent missed; fixed inline to `.map_or_else(...)` before commit); both binaries no-op rebuild gate-fresh; DATA-LOSS sentinel byte-identical (43 files: modlists.json + 42 per-modlist workspace.json) across agent's + orchestrator's full gate sequence; 6 render-gate PNGs under `target/render_gate/popup_collapse_and_radial_bg/` confirm the chevron `▼` renders on the title bar (carve-out #2 contract in effect). **Known shortfall in PR body:** 6 PNGs instead of brief's 16 — the 7 BIO popup surfaces need live `WizardState` with parsed scan data to render meaningfully; the agent substituted a representative `egui::Window` with `.collapsible(true)`; live-app manual test (per PR body's manual-test script) is the canonical visual check for the 7 actual popups.
...
## Fix-set ship log + remaining open risks
...
**Item #4 Create-fork post-route UX — arc closed, merged (PR #8)** (branch `xgatt/create-then-modify-fix`, single-branch arc, six runs). Both 🔴 data-loss bugs surfaced by the workflow-trace audit closed by Run 6.
