...
pub(super) fn parse_forbid_component(upper: &str, raw: &str, line: usize) -> Option<Tp2Rule> {
...
pub(super) fn parse_action_if_mod_missing(upper: &str, raw: &str, line: usize) -> Option<Tp2Rule> {
    if !upper.contains("ACTION_IF") || !upper.contains("MOD_IS_INSTALLED") {
        return None;
    }
    let targets = parse_all_mod_is_installed_by_negation(upper, raw, true);
...
pub(super) fn parse_mod_component_optional(after: &str) -> Option<(String, Option<u32>)> {
    let trimmed = after.trim_start();
...
pub(super) fn parse_all_mod_is_installed(upper: &str, raw: &str) -> Vec<(String, Option<u32>)> {
...
pub(super) fn parse_all_mod_is_installed_by_negation(
    upper: &str,
    raw: &str,
    want_negated: bool,
...
pub(super) fn parse_token_group(input: &str) -> Vec<String> {
...
pub(super) fn normalize_tp2_ref(value: &str) -> String {
