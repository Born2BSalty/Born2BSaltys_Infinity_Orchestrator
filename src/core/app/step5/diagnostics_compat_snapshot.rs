...
fn append_step2_compat_snapshot(tab: &str, mods: &[Step2ModState], out: &mut String) {
    ...
    writeln!(out, "{tab}:").expect("writing to String should not fail");
...
fn append_step2_component(
    ...
    {
        writeln!(out, "    reason: {reason}").expect("writing to String should not fail");
    }
    if let Some(source) = component.compat_source.as_deref()
        && !source.trim().is_empty()
    {
        writeln!(out, "    source: {source}").expect("writing to String should not fail");
    }
    if let Some(related_mod) = component.compat_related_mod.as_deref() {
        ...
        writeln!(out, "    related: {related}").expect("writing to String should not fail");
    }
}
