...
pub(super) fn run_write_checks(state: &WizardState, ts: u64) -> WriteCheckSummary {
    ...
    push_directory_checks(s, ts, &mut seen, &mut summary);
    push_binary_and_log_file_checks(s, ts, &mut seen, &mut summary);
...
fn push_directory_checks(
    s: &Step1State,
    ts: u64,
    seen: &mut HashSet<String>,
    summary: &mut WriteCheckSummary,
) {
    push_dir_check("Mods Folder", &s.mods_folder, ts, seen, summary);
    push_dir_check(
        "Generate Directory",
    ...
    push_dir_check("WeiDU log folder", &s.weidu_log_folder, ts, seen, summary);
    push_dir_check("BGEE game folder", &s.bgee_game_folder, ts, seen, summary);
    push_dir_check("BG2EE game folder", &s.bg2ee_game_folder, ts, seen, summary);
    push_dir_check(
    ...
    push_dir_check("BGEE log folder", &s.bgee_log_folder, ts, seen, summary);
    push_dir_check("BG2EE log folder", &s.bg2ee_log_folder, ts, seen, summary);
    push_dir_check(
        "EET BGEE log folder",
    ...
    push_dir_check("EET pre directory", &s.eet_pre_dir, ts, seen, summary);
    push_dir_check("EET new directory", &s.eet_new_dir, ts, seen, summary);
}

fn push_binary_and_log_file_checks(
    s: &Step1State,
    ts: u64,
    seen: &mut HashSet<String>,
    summary: &mut WriteCheckSummary,
    ...
    push_file_check("WeiDU binary", &weidu_binary, ts, seen, summary);
    ...
    push_file_check("WeiDU log file", &s.log_file, ts, seen, summary);
    push_file_check("BGEE WeiDU log file", &s.bgee_log_file, ts, seen, summary);
    push_file_check("BG2EE WeiDU log file", &s.bg2ee_log_file, ts, seen, summary);
}
