...
pub(crate) fn try_auto_answer_prompt(
    ...
    let ready_for_scripted =
        super::readiness::ready_for_scripted(state, now_ms, scripted_debounce_ms);
    let ready_for_fallback =
        super::readiness::ready_for_fallback(state, now_ms, headers_ready, fallback_debounce_ms);
    ...
    let settle_active = super::readiness::settle_active(state, now_ms, post_send_delay_ms);
    if settle_active {
        return;
    }
    if has_scripted_candidate
        && super::scripted::send_scripted(
            state,
            term,
            &prompt_key,
            &preview_line,
    ...
    let allow_json_fallback = super::scripted::allow_json_fallback_after_scripted(
        state,
        term,
        &prompt_key,
        now_ms,
    ...
    let _ = super::scripted::try_send_json_fallback(
        state,
        term,
        &prompt_key,
        legacy_key.as_deref(),
