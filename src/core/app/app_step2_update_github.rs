...
pub(super) fn check_github_download_page(
    ...
    if request.channel.is_some() || request.pkg.is_some() {
...
fn check_github_modhub_download_page(
    ...
    if request.pkg.is_some() && matches!(channel, GitHubChannel::Master | GitHubChannel::IFeelLucky)
    {
        return failed_outcome(
            request.clone(),
            "GitHub channel does not support pkg selection",
        );
    }
