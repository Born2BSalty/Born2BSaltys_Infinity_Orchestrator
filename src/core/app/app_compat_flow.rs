...
fn map_issues(result: &CompatValidationResult) -> Vec<CompatIssueDisplay> {
    result
        ...
        .map(|i| CompatIssueDisplay {
            ...
            component_block: i.component_block.clone(),
        })
        .collect()
}
}
...
pub(super) fn run_validation_for_step2_checked_order(
    ...
    match game {
        ...
        "BGEE" => {
            ...
            let result = validator.validate_step2_selection(&selected, "BGEE");
            map::to_state(result)
        }
        _ => {
            let selected = select::build_selected_components_from_step2(&state.step2.bg2ee_mods);
            let result = validator.validate_step2_selection(&selected, "BG2EE");
            map::to_state(result)
        }
    }
}
