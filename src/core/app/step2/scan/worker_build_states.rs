...
pub(super) fn to_mod_states(
    ...
    let mut mods: Vec<Step2ModState> = map
        .into_iter()
        .map(|(group_key, comps)| mod_state_from_group(&group_key, comps, tp2_map, mods_root))
        .collect();

    mods.retain(|mod_state| !should_drop_hidden_only_utility_mod(mod_state));
    disambiguate_duplicate_mod_names(&mut mods, mods_root);

    mods
}

fn mod_state_from_group(
    group_key: &str,
    ...
    let hidden_components =
        hidden_components_for(&deduped_components, &hidden_prompt_like_component_ids);
    ...
    let mod_prompt_summary = first_mod_prompt_summary(&deduped_components);
    let mod_prompt_events = first_mod_prompt_events(&deduped_components);
    ...
    Step2ModState {
        ...
        checked: false,
        hidden_components,
        components: component_states_for(
            deduped_components,
...
fn hidden_components_for(
    components: &[ScannedComponent],
    hidden_prompt_like_component_ids: &HashMap<String, String>,
...
fn first_mod_prompt_summary(components: &[ScannedComponent]) -> Option<String> {
    components
        .iter()
...
fn first_mod_prompt_events(
    components: &[ScannedComponent],
) -> Vec<crate::parser::PromptSummaryEvent> {
    components
        .iter()
        .find_map(|component| {
...
fn component_states_for(
    components: Vec<ScannedComponent>,
    derived_weidu_groups: &HashMap<String, String>,
    derived_collapsible_groups: &HashMap<String, groups::DerivedCollapsibleGroup>,
    ...
    components
        ...
        .map(|component| {
            ...
            let derived_group = derived_collapsible_groups
                ...
                .cloned();
            Step2ComponentState {
                is_meta_mode_component: meta_mode_component_ids.contains(component_id.as_str()),
                component_id: component.component_id,
                label: component.display,
                weidu_group: derived_weidu_groups.get(component_id.as_str()).cloned(),
...
fn disambiguate_duplicate_mod_names(mods: &mut [Step2ModState], mods_root: &Path) {
    let mut counts: HashMap<String, usize> = HashMap::new();
    for mod_state in mods.iter() {
        *counts
