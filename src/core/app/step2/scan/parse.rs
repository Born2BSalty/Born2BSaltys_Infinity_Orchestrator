...
pub fn dedup_components(comps: Vec<ScannedComponent>) -> Vec<ScannedComponent> {
    ...
    let mut first_index_by_key = HashMap::<(String, String), usize>::new();
    ...
    for component in comps {
        ...
        if let Some(existing_idx) = first_index_by_key.get(&key).copied() {
            if is_better_component_label(&out[existing_idx].display, &component.display) {
                out[existing_idx] = component;
            }
        } else {
            first_index_by_key.insert(key, out.len());
            out.push(component);
        }
    }
