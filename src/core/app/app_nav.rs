...
pub(super) fn can_advance_from_current_step(app: &WizardApp) -> bool {
    ...
    match app.state.current_step {
        0 => app.state.is_step1_valid() && matches!(app.state.step1_path_check, Some((true, _))),
...
fn step2_has_selection(app: &WizardApp) -> bool {
    ...
    match app.state.step1.game_install.as_str() {
        ...
        "EET" => {
            selected_in(&app.state.step2.bgee_mods) || selected_in(&app.state.step2.bg2ee_mods)
        }
...
fn step3_has_items(app: &WizardApp) -> bool {
    let real_items_in =
        |items: &[crate::ui::state::Step3ItemState]| -> bool { items.iter().any(|i| !i.is_parent) };
    match app.state.step1.game_install.as_str() {
        "BG2EE" => real_items_in(&app.state.step3.bg2ee_items),
        "EET" => {
            real_items_in(&app.state.step3.bgee_items)
                || real_items_in(&app.state.step3.bg2ee_items)
        }
