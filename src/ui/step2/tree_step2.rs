...
pub(crate) mod step2_tree {
    pub(crate) mod render_helpers {
        ...
        pub(crate) fn enforce_subcomponent_single_select(
            ...
            let Some(base_key) = subcomponent_base_key(&mod_state.components[changed_idx].label)
            else {
                return;
            };
        ...
        pub(crate) fn enforce_meta_mode_exclusive(
            mod_state: &mut Step2ModState,
            changed_idx: usize,
        ) {
        ...
        pub(crate) fn parent_compat_summary(
            ...
            if missing_deps > 0 {
                return Some((
                    ...
                    crate::ui::shared::theme_global::info_fill(),
                    format!(
                        "{missing_deps} missing dep{}",
                        if missing_deps == 1 { "" } else { "s" }
                    ),
                ));
            }
            if game_mismatch > 0 {
    ...
    mod render {
        ...
        pub fn render_mod_tree(
            ...
            if new_selection.is_some()
                || open_compat_for_component.is_some()
                || open_prompt_popup.is_some()
            {
