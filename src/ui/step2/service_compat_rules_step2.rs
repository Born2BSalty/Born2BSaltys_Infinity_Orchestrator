...
pub(crate) fn export_compat_report(
    step2: &Step2State,
    compat: &CompatState,
) -> io::Result<PathBuf> {
...
mod compat_matcher {
    ...
    fn rule_matches_component_text(
        rule: &Step2CompatRule,
        component: &Step2ComponentState,
    ) -> bool {
    ...
    fn normalize_tp2_stem(value: &str) -> String {
        ...
        stem.strip_prefix("setup-").unwrap_or(stem).to_string()
    }
}
...
mod compat_apply {
    ...
    fn path_requirement_fallback_message(rule: &Tp2Rule) -> String {
        ...
        if *must_exist {
            format!(
                "This component requires {kind_label} '{path}' to exist in the target game directory."
            )
        } else {
            format!(
                "This component requires {kind_label} '{path}' to be absent from the target game directory."
            )
        }
    }
