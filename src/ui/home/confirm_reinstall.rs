...
pub fn render(
    ...
    egui::Window::new(format!("Reinstall \"{}\"?", entry.name))
        .collapsible(false)
        .resizable(false)
        ...
        .show(ctx, |ui| {
            ui.label(
                egui::RichText::new("This will erase the current install folder and re-run the entire install from scratch. Your component selection and order are preserved; the modlist moves back to in-progress while the install runs, then returns to installed when complete.")
            ...
            ui.horizontal(|ui| {
                ...
                ui.label(
                    egui::RichText::new("• existing files at:")
                        .size(REDESIGN_LABEL_FONT_SIZE_PX)
            ...
            ui.horizontal(|ui| {
                ui.add_space(REDESIGN_MODAL_LIST_INDENT_PX);
                ui.label(
                    egui::RichText::new("will be deleted")
                        .size(REDESIGN_LABEL_FONT_SIZE_PX)
                        .color(redesign_text_muted(palette)),
                );
            });
            ...
            ui.horizontal(|ui| {
                if redesign_btn(ui, palette, "Cancel", BtnOpts::default()).clicked() {
                    action = Some(HomeAction::CancelReinstall);
                }
                if redesign_btn(
                    ui,
                    palette,
                ...
                .clicked()
                {
                    action = Some(HomeAction::ConfirmReinstallIntent);
                }
            });
        });
