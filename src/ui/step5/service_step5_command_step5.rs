...
pub fn build_resume_invocation(step1: &Step1State, resume_targets: &ResumeTargets) -> (String, Vec<String>) {
    ...
    let installer = installer_program(step1);
    if step1.game_install == "EET" {
        let bg1_dir = resume_targets.bg1_game_dir.as_deref().unwrap_or(step1.eet_bgee_game_folder.trim());
        let bg2_dir = resume_targets.bg2_game_dir.as_deref().unwrap_or(step1.eet_bg2ee_game_folder.trim());
        args.push("eet".to_string());
        args.push("--bg1-game-directory".to_string());
