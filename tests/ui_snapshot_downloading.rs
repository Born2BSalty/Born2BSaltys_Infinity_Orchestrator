...
fn representative_live_progress() -> DownloadProgress {
    let row = |name: &str,
        ...
        status,
        per_byte,
        expected_size: per_byte.and_then(|(_, total)| total),
    };
...
fn render_cell(out_dir: &Path, cell: Cell) -> PathBuf {
    ...
    let path = out_dir.join(format!("downloading_live__{}.png", cell.label));
    img.save(&path)
        .unwrap_or_else(|e| panic!("write PNG {}: {e}", path.display()));

    let abs = path.canonicalize().unwrap_or_else(|_| path.clone());
    println!(
        "SNAPSHOT  {}  downloading-live  -> {}",
        cell.label,
        abs.display()
    );
    path
