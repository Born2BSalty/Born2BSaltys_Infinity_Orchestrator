...
fn render_dl_window_two_phase_matrix() {
    ...
    assert_written(&written, 6, "expected 6 PNGs (2 phase scenes x 3 widths)");
}
...
fn render_scene_cell(out_dir: &Path, scene: Scene, cell: Cell) -> PathBuf {
    ...
    let path = out_dir.join(format!("dl_window_{}__{}.png", scene.name, cell.label));
    img.save(&path)
