...
impl<'a> TerminalView<'a> {
    ...
    fn process_input(
        ...
        // Keyboard/IME input should work whenever terminal has focus,
        // not only while the mouse cursor is inside the terminal rect.
        if !layout.has_focus() {
            return self;
        }
    ...
    fn show(
        ...
        let total_lines = content.grid.total_lines();
        let viewport_lines = content.grid.screen_lines();
        ...
        if max_offset > 0 {
            let track_rect = Rect::from_min_max(
                ...
                Pos2::new(layout_max.x, layout_max.y),
            );
            let track_h = track_rect.height().max(1.0);
            let thumb_h = (track_h * (viewport_lines as f32 / total_lines as f32))
                .max(SCROLLBAR_MIN_THUMB)
                .min(track_h);
            let range_h = (track_h - thumb_h).max(1.0);
            // Alacritty display_offset semantics: 0 = live bottom, max_offset = oldest top.
            // We map that so live bottom renders with thumb at the bottom (like standard consoles).
            let normalized = 1.0 - (display_offset as f32 / max_offset as f32);
            let thumb_top = track_rect.top() + normalized * range_h;
            let thumb_rect = Rect::from_min_size(
                Pos2::new(track_rect.left(), thumb_top),
            ...
            shapes.push(Shape::Rect(RectShape::filled(
                track_rect,
                CornerRadius::ZERO,
                global_bg.linear_multiply(0.7),
            )));
            shapes.push(Shape::Rect(RectShape::filled(
                thumb_rect,
                CornerRadius::same(2),
                self.theme
                    .get_color(Color::Named(NamedColor::BrightBlack))
                    .linear_multiply(1.2),
            )));

            let (is_down, is_released, pointer_pos) = layout.ctx.input(|i| {
                (
            ...
            if let Some(pos) = pointer_pos {
                if is_down && thumb_rect.contains(pos) {
                    state.scrollbar_dragged = true;
                }
                if is_down && state.scrollbar_dragged {
                    let normalized = ((pos.y - track_rect.top()) / track_h).clamp(0.0, 1.0);
                    let target = ((1.0 - normalized) * max_offset as f32).round() as usize;
                    self.backend.process_command(BackendCommand::ScrollTo(target));
                }
            }
        }
...
fn process_left_button(
    ...
    if pressed && is_in_scrollbar(layout, position) {
        state.scrollbar_dragged = true;
        return InputAction::Ignore;
    }
    if !pressed && state.scrollbar_dragged {
        state.scrollbar_dragged = false;
        return InputAction::Ignore;
    }
    let terminal_mode = backend.last_content().terminal_mode;
...
fn is_in_scrollbar(layout: &Response, position: Pos2) -> bool {
    let track_rect = Rect::from_min_max(
        Pos2::new(layout.rect.max.x - SCROLLBAR_WIDTH, layout.rect.min.y),
        layout.rect.max,
    );
    track_rect.contains(position)
}
